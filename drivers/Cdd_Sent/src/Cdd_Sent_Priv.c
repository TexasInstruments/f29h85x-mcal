...
P2VAR(Cdd_Sent_DriverObjType, CDD_SENT_VAR_FAST_NO_INIT, CDD_SENT_VAR_FAST_NO_INIT) Cdd_Sent_DrvObjPtr;
 ...
 *
 *********************************************************************************************************************/
static FUNC(void, CDD_SENT_CODE) Cdd_Sent_SetCRCConfigPriv(
    P2CONST(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_CONST) ConfigPtr);
...
/** \brief Cdd_Sent_ProcessIsrFastInterrupts : Process the Fast Channel Interrupts when Cdd_Sent_ProcessISR is called.
 *
 ...
 *
 *********************************************************************************************************************/
static FUNC(void, CDD_SENT_CODE) Cdd_Sent_ProcessIsrFastInterrupts( \
    P2VAR(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_APPL_DATA) Sent_Instance_Object, \
    const CddSent_ValueType instance_index, \
    uint32 Interrupt_Status_Register);

/** \brief Cdd_Sent_ProcessIsrSlowInterrupts : Process the Slow Channel Interrupts when Cdd_Sent_ProcessISR is called.
 *
 * \param[in] Sent_Instance_Object Configuration pointer to Sent HW Unit.
 ...
 *
 *********************************************************************************************************************/
static FUNC(void, CDD_SENT_CODE) Cdd_Sent_ProcessIsrSlowInterrupts(
    P2VAR(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_APPL_DATA) Sent_Instance_Object,
    Cdd_SentInstance SentInstance);

/** \brief Cdd_Sent_ProcessIsrCallCddSentUserErrorCallbackFunction : Calls CddSentUserErrorCallbackFunction.
 *
 * \param[in] Sent_Instance_Object Configuration pointer to Sent HW Unit.
 * \pre None
 * \post None
 ...
 *
 *********************************************************************************************************************/
static FUNC(void, CDD_SENT_CODE) Cdd_Sent_ProcessIsrCallCddSentUserErrorCallbackFunction(
    P2VAR(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_APPL_DATA) Sent_Instance_Object);

/** \brief Cdd_Sent_SetHandleIdPriv : Set the Handle Id for all Channels of Sent HW Unit whose Channel Type and Sensor
 *                                Type are valid.
 *
 ...
 *
 *********************************************************************************************************************/
static FUNC(void, CDD_SENT_CODE) Cdd_Sent_SetHandleIdPriv(
    P2VAR(Cdd_Sent_DriverObjType, AUTOMATIC, CDD_SENT_APPL_DATA) Cdd_Sent_DriverObj,
    P2VAR(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_APPL_DATA) Sent_Hw_Unit,
    uint32 loop_count);
/*********************************************************************************************************************
...
Cdd_Sent_HwInitPrv(P2CONST(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_CONST) ConfigPtr)
{
        ...
        for(uint32 DataMap=(uint32)CDD_SENT_DATAX_NIB0; DataMap<=(uint32)CDD_SENT_DATAX_NIB7;DataMap++)
        {
            if(DataNibblesCount > 0x0U)
            {
                    Cdd_Sent_setDataSortingFormat(BaseAddress, \
                                        CDD_SENT_RECEIVER_DATA0, DataMap, DataNibblesCount);
                    DataNibblesCount=DataNibblesCount - 0x1U;
            }
        }
        Cdd_Sent_setDataSortingFormat(BaseAddress, \
                                        CDD_SENT_RECEIVER_DATA1,\
                                        CDD_SENT_CRC_NIBBLE_PS_0, CDD_SENT_CRC_NIBBLE_1);
        Cdd_Sent_setDataSortingFormat(BaseAddress, \
                                        CDD_SENT_RECEIVER_DATA1,\
                                        CDD_SENT_CRC_NIBBLE_PS_1, CDD_SENT_CRC_NIBBLE_2);
        Cdd_Sent_setDataSortingFormat(BaseAddress, \
                                        CDD_SENT_RECEIVER_DATA1,\
                                        CDD_SENT_CRC_NIBBLE_PS_2, CDD_SENT_CRC_STATUS_NIBBLE);
        ...
        if (TRUE == ConfigPtr->CddSentAcceptErrorData)
        {
            ...
            McalLib_RegBitClear32((BaseAddress + SENT_O_RCFG2),
                                    SENT_RCFG2_NOWR_ERRDATA);
        }
        else
        {
            /* Disable Error date to written into memory */
            McalLib_RegBitSet32((BaseAddress + SENT_O_RCFG2),
                                    SENT_RCFG2_NOWR_ERRDATA);
        }
...
Cdd_Sent_MTPInitPrv(P2CONST(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_CONST) ConfigPtr)
{
    ...
    for(Sensor_Count = 0U; Sensor_Count < ConfigPtr->CddSentMTPChannelCount; Sensor_Count++)
    {
        ...
        if(TRUE == MTPSensorConfig->CddSentMTPSensorEnable)
        {
            ...
            Cdd_Sent_setMTPPeriod(ConfigPtr->CddSentBaseAddress,
                                MTPSensorConfig->CddSentMTPSensorType, 
                                MTPSensorConfig->CddSentMTPSensorPeriod);

            MtptriggerOffset = (SENT_MTP_SWTR_STEP * (uint32)MTPSensorConfig->CddSentMTPSensorType) + \
                                SENT_O_BC_TRIGSEL;
            ...
            switch(MTPSensorConfig->CddSentMTPSensorType)
            {
                ...
                case CDD_SENT_CHANNEL_SENSOR_1:
                    ...
                    Cdd_Sent_configureMTPTimeout(ConfigPtr->CddSentBaseAddress,\
                    CDD_SENT_CHANNEL_SENSOR_1,MTPSensorConfig->CddSentMTPSensorTimeout);
                ...
                case CDD_SENT_CHANNEL_SENSOR_2:
                    ...
                    Cdd_Sent_setSensor2MTPToggleTime(ConfigPtr->CddSentBaseAddress, 10,
                                        MTPSensorConfig->CddSentMTPSensorCompare10);
                    Cdd_Sent_configureMTPTimeout(ConfigPtr->CddSentBaseAddress,\
                    CDD_SENT_CHANNEL_SENSOR_2,MTPSensorConfig->CddSentMTPSensorTimeout);
                ...
                case CDD_SENT_CHANNEL_SENSOR_3:
                    ...
                    Cdd_Sent_setSensor3MTPToggleTime(ConfigPtr->CddSentBaseAddress, 10,
                                        MTPSensorConfig->CddSentMTPSensorCompare10);
                    Cdd_Sent_configureMTPTimeout(ConfigPtr->CddSentBaseAddress,
                    CDD_SENT_CHANNEL_SENSOR_3,MTPSensorConfig->CddSentMTPSensorTimeout);
                ...
                case CDD_SENT_CHANNEL_SENSOR_4:
                    ...
                    Cdd_Sent_setSensor4MTPToggleTime(ConfigPtr->CddSentBaseAddress, 10,
                                        MTPSensorConfig->CddSentMTPSensorCompare10);
                    Cdd_Sent_configureMTPTimeout(ConfigPtr->CddSentBaseAddress,
                    CDD_SENT_CHANNEL_SENSOR_4,MTPSensorConfig->CddSentMTPSensorTimeout);
            ...
            P2CONST(PduInfoType, AUTOMATIC, CDD_SENT_CONST) PduInfoPtr)
{
    ...
    triggerOffset = (SENT_MTP_SWTR_STEP * (uint32)Channel_Id) + \
                     SENT_O_BC_TRIGSEL;
 ...
 *Design: MCAL-28676
 */
FUNC(void, CDD_SENT_CODE) Cdd_Sent_GetChIdxPriv(const Cdd_Sent_DriverObjType *CddSentDrvObj, \
                                            Cdd_SentInstance SentInstance, uint32* InstanceIndex)
 {
...
Cdd_Sent_HwDenitPrv(P2CONST(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_CONST) ConfigPtr)
{
    ...
    McalLib_RegBitSet32((Base_Address + SENT_O_CSENT_SWR),
                                SENT_CSENT_SWR_SWR);
    /* Reset Sent  Mtp Registers */
    McalLib_RegBitSet32((Base_Address + SENT_O_MTP_SWR),
                                SENT_MTP_SWR_SWR);
}
...
void Cdd_Sent_ProcessISR(Cdd_SentInstance SentInstance)
{
    ...
    if (SENT_MAX_HW_UNITS > instance_index)
    {
        ...
        uint32 Enable_Fast_Interrupts= (SENT_REINT_RFIFO_TRIGGER_E | SENT_REINT_RFAST_S1DV_E |
            SENT_REINT_RFAST_S2DV_E |
            SENT_REINT_RFAST_S3DV_E |
            SENT_REINT_RFAST_S4DV_E) & Interrupt_Enable_Register;
...
{
    VAR(uint32, AUTOMATIC) Sent_Base = ((uint32)SENT1CSENT_BASE) + (((uint32)SentInstance) * 0x1000U);
                            ...
                            uint32 sensorTimeout)
{
    ...
    sensorOffset = (SENT_MTP_TIMEOUT_STEP * (uint32)sensorNum) + \
                    SENT_O_S1_MTP_TO;
...
Cdd_Sent_setMTPWaitTime(uint32 mtpBase, uint32 waitTime)
{
    ...
    HWREG(mtpBase + SENT_O_WAITTIME) = (HWREG(mtpBase + SENT_O_WAITTIME) & \
 ...
 *Design: MCAL-28837
 */
FUNC(Std_ReturnType, CDD_SENT_CODE) Cdd_Sent_TransmitPriv(P2CONST(PduInfoType, AUTOMATIC, CDD_SENT_CONST) PduInfoPtr,
                                 PduIdType TxPduId)
{
    ...
    {
        for(Sensor_Count = (uint8)0U; Sensor_Count < \
        Cdd_Sent_DrvObjPtr->CddSent_CfgPtr->Cdd_Sent_HWUnit[loop_count]->CddSentMTPChannelCount; Sensor_Count++)
        {
            if(Cdd_Sent_DrvObjPtr->CddSent_CfgPtr->Cdd_Sent_HWUnit[loop_count]->\
            CddSentMTPConfigList[Sensor_Count]->CddSentMTPSensorPduID == TxPduId)
            {
                Instance_Id = Cdd_Sent_DrvObjPtr->CddSent_CfgPtr->Cdd_Sent_HWUnit[loop_count]->CddSentInstance;
                Channel_Id = Cdd_Sent_DrvObjPtr->CddSent_CfgPtr->\
                Cdd_Sent_HWUnit[loop_count]->CddSentMTPConfigList[Sensor_Count]->CddSentMTPSensorType;
                Trigger_Source = Cdd_Sent_DrvObjPtr->CddSent_CfgPtr->\
                Cdd_Sent_HWUnit[loop_count]->CddSentMTPConfigList[Sensor_Count]->CddSentTriggerSource;
    ...
    P2CONST(Cdd_Sent_HWUnitType, AUTOMATIC, CDD_SENT_CONST) ConfigPtr)
{
    ...
    {
        /* Enable CRC */
        McalLib_RegBitSet32(BaseAddress + SENT_O_RCFG,
                    SENT_RCFG_RX_CRCENB);

        /* Configuring CRC Type */
        McalLib_RegMFWriteRaw32((BaseAddress + SENT_O_RCFG),
                                SENT_RCFG_RX_CRCTYPE,0,
                                ConfigPtr->CddSentCRCType);
    ...
    {
        /* Disable Pause Pulse */
        McalLib_RegBitClear32((BaseAddress + SENT_O_RCFG),
                                SENT_RCFG_RX_PPENB);
    }
    else
    {
    ...
    uint32 Interrupt_Status_Register)
{
    ...
    VAR(uint32, AUTOMATIC) FIFOTriggerLevel = HWREG(Sent_Instance_Object->CddSentBaseAddress+0x34U) >> SENT_RCFG2_RFIFO_TRIGLEV_S ;
    VAR(Cdd_Sent_Data_Buffer, AUTOMATIC) Fast_Buffer_Object = Sent_Instance_Object->CddSent_Buffer_Data;
    if(Sent_Instance_Object->CddSentMTP==TRUE)
    {
        ...
        if(Sent_Instance_Object->CddSentUserCallbackFunction != NULL_PTR)
        {
            ...
            /* When Cdd Sent is integrated with Autosar Com stack, this parameter is a Pdu identifier 
            When Cdd Sent is not integrated with Autosar Com stack, this parameter is a channel object identifier */
    ...
    Cdd_SentInstance SentInstance)
{
    ...
    for(uint32 Channel_count = 0U; Channel_count < Sent_Instance_Object->CddSentChannelCount; Channel_count++)
    {
        ...
        if(((uint32) Sent_Instance_Object->CddSentChannelConfigList[Channel_count]->\
        CddSentMessageID) == ((uint32)Cdd_Sent_getMessageID(SentInstance)))
        {
