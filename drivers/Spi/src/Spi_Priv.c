 ...
 *
 ********************************************************************************************************************/
static FUNC(void, SPI_CODE) Spi_Start(P2CONST(Spi_HwUnitObjType,AUTOMATIC, SPI_CODE)hwUnitObj, \
               P2CONST(Spi_JobObjType,AUTOMATIC, SPI_CODE)jobObj, \
               P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)ChObj, \
 ...
 *
 ********************************************************************************************************************/
static FUNC(Spi_JobResultType, SPI_CODE) Spi_ContinueTxRxFifoDisable(P2VAR(Spi_HwUnitObjType,AUTOMATIC, SPI_CODE)hwUnitObj,
                                   P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj);
 ...
 *
 ********************************************************************************************************************/
static FUNC(Spi_JobResultType, SPI_CODE) Spi_ContinueTxRxFifoEnabled(P2VAR(Spi_HwUnitObjType,AUTOMATIC, SPI_CODE)hwUnitObj,
                                   P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj);
 ...
 *
 ********************************************************************************************************************/
static FUNC(Spi_JobResultType, SPI_CODE) Spi_ProcessRxFifoEnable(P2VAR(Spi_HwUnitObjType,AUTOMATIC, SPI_CODE)hwUnitObj,
                                   P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj,
                                   P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj);                                   
 ...
 *
 ********************************************************************************************************************/
static FUNC(Spi_JobResultType, SPI_CODE) Spi_ProcessRxFifoDisable(P2VAR(Spi_HwUnitObjType,AUTOMATIC, SPI_CODE)hwUnitObj,
                                   P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj,
                                    P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj);
 ...
 *
 ********************************************************************************************************************/
static FUNC(void, SPI_CODE) Spi_PrivEndNotification(Spi_SeqEndNotifyType seqEndNotification,
...
/* Design: MCAL-28312 */
LOCAL_INLINE void Spi_FifoWrite8(VAR(uint32,AUTOMATIC) baseAddr,\
                                       P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj,
...
/* Design: MCAL-28313 */
LOCAL_INLINE void Spi_FifoWrite16(VAR(uint32,AUTOMATIC) baseAddr,\
                                      P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj,\
                                       VAR(uint32,AUTOMATIC) transferLength,\
                                       VAR(uint16,AUTOMATIC) curTxWords)
{
                                    ...
                                    P2VAR(Spi_HwUnitObjType,AUTOMATIC,SPI_CODE)hwUnitObj)
{
    ...
    McalLib_RegBitSet16(hwUnitObj->baseAddr + SPI_O_CTL,
 ...
 * Design : MCAL-28295  
 */
FUNC(Std_ReturnType, SPI_CODE) Spi_StartSeqAsync(P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj, \
                                                 P2VAR(Spi_SeqObjType,AUTOMATIC,SPI_CODE)seqObj)
{
    VAR(uint32, AUTOMATIC) index;
                   ...
                   P2CONST(Spi_JobObjType,AUTOMATIC, SPI_CODE)jobObj)
{
    ...
    McalLib_RegMFWriteRaw16(hwUnitObj->baseAddr + SPI_O_BRR, SPI_BRR_SPI_BIT_RATE_M,
    ...
    {
        McalLib_RegMFWriteRaw16(hwUnitObj->baseAddr + SPI_O_FFCT, SPI_FFCT_TXDLY_M,
                       ...
                       P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj)
{
    ...
    McalLib_RegMFWriteRaw16(baseAddr + SPI_O_CCR, SPI_CCR_SPICHAR_M, SPI_CCR_SPICHAR_S, (uint16)chObj->chCfg->dataWidth - 1U);
    ...
    if((chObj->numWordsTxRx > chObj->curTxWords)){
        ...
        if((boolean)TRUE == hwUnitObj->hwUnitCfg->fifoModeEnable){
            ...
            if(chObj->numWordsTxRx <= SPI_FIFO_RX_INTERRUPT_LEVEL){
                McalLib_RegMFWriteRaw16(baseAddr + SPI_O_FFRX, SPI_FFRX_RXFFIL_M, SPI_FFRX_RXFFIL_S, chObj->numWordsTxRx);
            } else{
                McalLib_RegMFWriteRaw16(baseAddr + SPI_O_FFRX, SPI_FFRX_RXFFIL_M, SPI_FFRX_RXFFIL_S, SPI_FIFO_RX_INTERRUPT_LEVEL);
            }
            /* Transmit next words */
                ...
                P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj)
{
    ...
    if((boolean)TRUE == hwUnitObj->hwUnitCfg->fifoModeEnable){
        /* TI_COVERAGE_GAP_START [Branch Coverage] number of numWordsTxRx words will be always 
           gereater than curTxWords as before calling this function this condition additional condition 
           prevents data overflow in case on re-entrance. This condition added to resolve MISCRA issue */
        if(chObj->numWordsTxRx > chObj->curTxWords){
            /* TI_COVERAGE_GAP_STOP */
            if(chObj->numWordsTxRx <= SPI_FIFO_RX_INTERRUPT_LEVEL){
                McalLib_RegMFWriteRaw16(baseAddr + SPI_O_FFRX, SPI_FFRX_RXFFIL_M, SPI_FFRX_RXFFIL_S, chObj->numWordsTxRx);
                                                            ...
                                                            P2VAR(Spi_SeqObjType,AUTOMATIC,SPI_CODE)seqObj)
{
    ...
    for (index = ((uint32) 0U); index < seqObj->seqCfg->jobPerSeq; index++)
    {
        ...
        /* Queue the job to the hardware queue */
        jobObj->seqObj = seqObj;
        /* TI_COVERAGE_GAP_START [Branch Coverage] As for loop will exit before numJobsPending reaching 0, 
           this condition will never become false. This condition added to resolve MISCRA issue */
                                 ...
                                 P2VAR(Spi_JobObjType,AUTOMATIC, SPI_CODE)jobObj)
{
    ...
    * different */
    ...
    for (index = ((uint32) 0U); index < jobObj->jobCfg->channelPerJob; index++)
    {
        ...
        do
        {
            McalLib_GetElapsedValue(&startCount,
                                &elapsedCount);
                               ...
                               VAR(uint32,AUTOMATIC)baseAddr)
{
    ...
    else
    {
        ...
        McalLib_RegWriteRaw16(baseAddr + SPI_O_TXBUF, \
        txData << (SPI_MAX_WORD_LENGTH - chObj->chCfg->dataWidth));
    }
    chObj->curTxWords++;
    return;
}
...
/* Design : MCAL-28296 */
FUNC(Std_ReturnType, SPI_CODE) Spi_StartSeqSync(P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj, \
                                                P2VAR(Spi_SeqObjType,AUTOMATIC,SPI_CODE)seqObj)
{
    VAR(Std_ReturnType, AUTOMATIC) retVal = ((Std_ReturnType) E_NOT_OK);
    {
             ...
             VAR(uint16,AUTOMATIC)txIntrStatus)
{
    ...
    if (chObj->numWordsTxRx > chObj->curTxWords)
    {
        ...
        /* TI_COVERAGE_GAP_START [Branch Coverage] fifo status cannot be greater than (SPI_FIFO_TX_FIFO_DEPTH - 1U), 
           this condition added to resolve MISCRA issue */
                                   ...
                                   P2VAR(Spi_ChannelObjType,AUTOMATIC,SPI_CODE)chObj)
{
    ...
    else if ((rxIntrStatus & SPI_FFRX_RXFFINT) == SPI_FFRX_RXFFINT)
    {
        ...
        #endif
        numWordsToRead = (uint32) McalLib_RegMFReadRaw16(baseAddr + SPI_O_FFRX, 
        ...
        ((chObj->numWordsTxRx - chObj->curRxWords) < SPI_FIFO_RX_INTERRUPT_LEVEL)){
            ...
            McalLib_RegMFWriteRaw16(baseAddr + SPI_O_FFRX, SPI_FFRX_RXFFIL_M, \
...
/* Design: MCAL-28321 */
FUNC(void, SPI_CODE) Spi_PrivInit(P2VAR(Spi_DriverObjType,AUTOMATIC,SPI_CODE) Spi_DrvObj, \
...
/* Design: MCAL-28346 */
static FUNC(void, SPI_CODE) Spi_PrivEndNotification(Spi_SeqEndNotifyType seqEndNotification,Spi_JobEndNotifyType jobEndNotification)
{
