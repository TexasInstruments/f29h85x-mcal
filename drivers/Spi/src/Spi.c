...
FUNC(void, SPI_CODE) Spi_GetVersionInfo(P2VAR(Std_VersionInfoType, AUTOMATIC, SPI_APPL_DATA) versioninfo)
{
    ...
    {
        (void)Det_ReportError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_GET_VERSION_INFO, 
		                                         SPI_E_PARAM_POINTER);
    }
    else
#endif  /* #if (STD_ON == SPI_DEV_ERROR_DETECT) */
...
FUNC(Spi_StatusType, SPI_CODE) Spi_GetHWUnitStatus(VAR(Spi_HWUnitType, AUTOMATIC) HWUnit)
{
    ...
    {
        (void)Det_ReportError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_GET_HW_UNIT_STATUS, 
                                                   SPI_E_PARAM_UNIT);
    }
    else
    #endif  /* #if (STD_ON == SPI_DEV_ERROR_DETECT) */
...
{   
    ...
    if (SPI_UNINIT == Spi_DrvStatus)
    {
       (void) Det_ReportError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_WRITE_IB, SPI_E_UNINIT);
             ...
             (Spi_DrvObj.channelObj[Channel].chCfg->channelBufType != 
			                                                SPI_IB))
    {
...

{
    ...
    {
        (void) Det_ReportError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_SETUP_EB, 
                                            SPI_E_PARAM_LENGTH);
    }
    else
#endif  /* #if (STD_ON == SPI_DEV_ERROR_DETECT) */
...
FUNC(Std_ReturnType, SPI_CODE) Spi_AsyncTransmit(VAR(Spi_SequenceType, AUTOMATIC) Sequence)
{
    ...
    #endif  /* #if (STD_ON == SPI_DEV_ERROR_DETECT) */
    {
       if (Spi_GetSequenceResult(Sequence) == SPI_SEQ_PENDING)
        {
          (void)  Det_ReportRuntimeError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_ASYNC_TRANSMIT, 
				                                  SPI_E_SEQ_PENDING);
        }
       else
        {
...
FUNC(void, SPI_CODE) Spi_MainFunction_Handling(void)
{
    ...
    {
        /* Note: This error return is not mentioned in spec */
       (void) Det_ReportError(SPI_MODULE_ID, SPI_INSTANCE_ID, SPI_SID_MAINFUNCTION_HANDLING, 
		                                               SPI_E_UNINIT);
    }
    else
  #endif  /* #if (STD_ON == SPI_DEV_ERROR_DETECT) */
...
static FUNC(Std_ReturnType, SPI_CODE) Spi_CheckConfig(const Spi_ConfigType *cfgPtr)
{
    ...
    for (index = ((uint32) 0U); index < cfgPtr->maxChannels; index++)
    {
        ...
        if ((cfgPtr->channelCfg[index].channelId >= cfgPtr->maxChannels) || 
            (cfgPtr->channelCfg[index].dataWidth < 1U))
        {
    ...
    if(((Std_ReturnType) E_OK) == retVal){
        ...
        {
            /* ID is used as index, can't exceed array size */
            if ((cfgPtr->seqCfg[index].seqId >= cfgPtr->maxSeq) ||
