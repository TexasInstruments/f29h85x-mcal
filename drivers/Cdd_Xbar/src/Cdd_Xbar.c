...
/* Design: MCAL-28364 */
/** \brief Checks for DET error related to output line of input crossbar, output crossbar and CLB crossbar.
 ...
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_Type_Output_Valid_Firstgroup(VAR(uint8, AUTOMATIC) CrossbarType, 
                                                               P2CONST(uint8, AUTOMATIC, CDD_XBAR_CONST) DestLine,
                                                               VAR(uint8, AUTOMATIC) ServID);

/* Design: MCAL-28365 */
/** \brief Checks for DET error related to output line of EPWM crossbar, MINDB crossbar and ICL crossbar.
 * \param[in] CrossbarType is the type of crossbar.
 * \param[in] DestLine is the output line of the crossbar instance.
 * \param[in] ServID is the service ID of the API being called.
 * \pre Preconditions - None.
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_Type_Output_Valid_Secondgroup(VAR(uint8, AUTOMATIC) CrossbarType, 
                                                               P2CONST(uint8, AUTOMATIC, CDD_XBAR_CONST) DestLine,
                                                               VAR(uint8, AUTOMATIC) ServID);

/* Design: MCAL-28366 */
/** \brief Checks for DET error related to input line of input crossbar, output crossbar and CLB crossbar.
 * \param[in] CrossbarType is the type of crossbar.
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_Type_Input_Valid_Firstgroup(VAR(uint8, AUTOMATIC) CrossbarType, 
                                                              P2CONST(uint16, AUTOMATIC, CDD_XBAR_CONST) InputLine,
                                                              VAR(uint8, AUTOMATIC) ServID);

/* Design: MCAL-28367 */
/** \brief Checks for DET error related to input line of EPWM crossbar, MINDB crossbar and ICL crossbar.
 * \param[in] CrossbarType is the type of crossbar.
 * \param[in] InputLine is the input line of the crossbar instance.
 * \param[in] ServID is the service ID of the API being called.
 * \pre Preconditions - None.
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_Type_Input_Valid_Secondgroup(VAR(uint8, AUTOMATIC) CrossbarType, 
                                                              P2CONST(uint16, AUTOMATIC, CDD_XBAR_CONST) InputLine,
                                                              VAR(uint8, AUTOMATIC) ServID);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_Output_Xbar_DetCheck(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                         VAR(uint8, AUTOMATIC) ServID);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_OutStretchPulseDetCheck 
(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(uint8, AUTOMATIC) ServID, 
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_InSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                  VAR(uint16, AUTOMATIC) InputLine,
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_OutSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                   VAR(uint16, AUTOMATIC) InputLine,
                                                                   VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_OutOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, 
                                                                         VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_EpwmSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                    VAR(uint16, AUTOMATIC) InputLine,
                                                                    VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_EpwmOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                          VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_ClbSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                   VAR(uint16, AUTOMATIC) InputLine,
                                                                   VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_ClbOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                         VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_MdlSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                   VAR(uint16, AUTOMATIC) InputLine,
                                                                   VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_MdlOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                         VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_IclSelect_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                   VAR(uint16, AUTOMATIC) InputLine,
                                                                   VAR(boolean, AUTOMATIC) Selection);
 ...
 * \retval E_NOT_OK if errors are not detected.
 *********************************************************************************************************************/
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_IclOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit,
                                                                         VAR(boolean, AUTOMATIC) Selection);
...
FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Init(P2CONST(Cdd_Xbar_ConfigType, AUTOMATIC, CDD_XBAR_CONFIG_DATA) ConfigPtr)
{
    ...
    if (CDD_XBAR_UNINIT != Cdd_Xbar_DriverStatus)
    {
        ...
        CDD_XBAR_SID_INIT, CDD_XBAR_E_INIT_ALREADY);
        #endif /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */ 
    }
    else if (NULL_PTR == config_ptr)
    {
        ...
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_INIT, CDD_XBAR_E_PARAM_POINTER);    
        #endif /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */   
    }
    else
    {
 ...
 */
     ...
     (0U < CDD_XBAR_EPWM_XBAR_CONFIGURATIONS) || (0U < CDD_XBAR_CLB_XBAR_CONFIGURATIONS) || \
     (0U < CDD_XBAR_MINDB_XBAR_CONFIGURATIONS) || (0U < CDD_XBAR_ICL_XBAR_CONFIGURATIONS))
FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_SelectInput(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, 
VAR(uint16, AUTOMATIC) InputLine, VAR(boolean, AUTOMATIC) Selection)
{
...
Cdd_Xbar_OutLatchSelect(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) LatchEnable)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_OUT_LATCH_SELECT, CDD_XBAR_E_LOCK);
        retval = E_NOT_OK;
    }
    else
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            Cdd_Xbar_OutputLatchSelect(&Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine, &LatchEnable);
        }
    }
...
(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_OUT_INV_BEFORE_LATCH, CDD_XBAR_E_LOCK);
        retval = E_NOT_OK;
    }
    else
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
 ...
 */
FUNC(Std_ReturnType, CDD_XBAR_CODE) \
Cdd_Xbar_OutStretchPulse(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(Cdd_Xbar_TickStretchType, AUTOMATIC) TickStretch)
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_OK;
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_OUTPUT_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_OutputStretchPulse
                (&Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine, &TickStretch);
        }
    }
...
FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_OutStretchPulseClear(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_OUT_STRETCH_PULSE_CLEAR, CDD_XBAR_E_LOCK);
        retval = E_NOT_OK;
    }
    else
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
...
Cdd_Xbar_SelectOutputInversion(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_SELECT_OUTPUT_INVERSION, CDD_XBAR_E_UNINIT);
    }
    else
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
...
FUNC(boolean, CDD_XBAR_CODE) Cdd_Xbar_CheckOutputInversion(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_CHECK_OUTPUT_INVERSION, CDD_XBAR_E_UNINIT);
    }
    else
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        switch (CDD_XBAR_GET_XBAR_TYPE(CrossbarUnit))
        {
            ...
            default:
                ...
                #if (STD_ON == CDD_XBAR_DEV_ERROR_DETECT)
                (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
                    CDD_XBAR_SID_LOCK, CDD_XBAR_E_PARAM_VALUE);
                #else
                retval = E_OK;
                #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
                break;
        ...
        switch (CDD_XBAR_GET_XBAR_TYPE(CrossbarUnit))
        {
            ...
            default:
              /* Type did not match any of the above.*/
                #if (STD_ON == CDD_XBAR_DEV_ERROR_DETECT)
                (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
                    CDD_XBAR_SID_LOCKSTATUS, CDD_XBAR_E_PARAM_VALUE);
                #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
                break;
        }
    }
...
FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_InputFlagClear(VAR(Cdd_Xbar_InputFlagType, AUTOMATIC) InputFlag)
{
    ...
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_INPUTFLAGCLEAR, CDD_XBAR_E_UNINIT);
    }
    else if (((CDD_XBAR_INPUT_FLAG_NUMBER(InputFlag)) > (uint16)18U) ||
        ((CDD_XBAR_INPUT_FLAG_BIT(InputFlag)) > (uint16)31U))
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, 
            CDD_XBAR_SID_INPUTFLAGCLEAR, CDD_XBAR_E_PARAM_VALUE);
    }
    else
#endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
...
VAR(Cdd_Xbar_TickStretchType, AUTOMATIC) TickStretch)
{
    ...
    else if ((TickStretch != CDD_XBAR_SYSTICKS_STRETCH_OFF) && 
        (TickStretch != CDD_XBAR_SYSTICKS_16) && (TickStretch != CDD_XBAR_SYSTICKS_32))
    {
        (void)Det_ReportError(CDD_XBAR_MODULE_ID, CDD_XBAR_INSTANCE_ID, ServID, CDD_XBAR_E_PARAM_VALUE);
    }
    else if (TRUE == Cdd_Xbar_OutLockStatus())
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Input_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_INPUT_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarInputXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_InputCfg[config].inputSelect, CDD_XBAR_SID_INIT)) ||
            ...
            &Cdd_Xbar_Config->Cdd_Xbar_InputCfg[config].inputSelect, CDD_XBAR_SID_INIT)))
        {
            ...
            &Cdd_Xbar_Config->Cdd_Xbar_InputCfg[config].inputLine, CDD_XBAR_SID_INIT)) || 
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Output_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_OUTPUT_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarOutputXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[config].outputLine, CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Output_Priv(VAR(uint8, AUTOMATIC) config)
{
    ...
    for (input = 0U; input < Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[config].input_count; input++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Input_Valid_Firstgroup(CddXbarConf_CddXbarOutputXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[config].inputLine[input], CDD_XBAR_SID_INIT)) ||
            (E_OK != Cdd_Xbar_Type_Input_Valid_Secondgroup(CddXbarConf_CddXbarOutputXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[config].inputLine[input], CDD_XBAR_SID_INIT)))
        {
            /* This function will throw DET for invalid inputs */
        }
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Epwm_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_EPWM_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarEpwmXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_EpwmCfg[config].outputLine, CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Epwm_Priv(VAR(uint8, AUTOMATIC) config)
{
    ...
    for (input = 0U; input < Cdd_Xbar_Config->Cdd_Xbar_EpwmCfg[config].input_count; input++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Input_Valid_Firstgroup(CddXbarConf_CddXbarEpwmXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_EpwmCfg[config].inputLine[input], CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Clb_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_CLB_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarClbXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_ClbCfg[config].outputLine, CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Clb_Priv(VAR(uint8, AUTOMATIC) config)
{
    ...
    for (input = 0U; input < Cdd_Xbar_Config->Cdd_Xbar_ClbCfg[config].input_count; input++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Input_Valid_Firstgroup(CddXbarConf_CddXbarClbXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_ClbCfg[config].inputLine[input], CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Mindb_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_MINDB_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarMindbXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_MindbCfg[config].outputLine, CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Mindb_Priv(VAR(uint8, AUTOMATIC) config)
{
    ...
    for (input = 0U; input < Cdd_Xbar_Config->Cdd_Xbar_MindbCfg[config].input_count; input++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Input_Valid_Firstgroup(CddXbarConf_CddXbarMindbXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_MindbCfg[config].inputLine[input], CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Icl_Xbar_Init_Priv(void)
{
    ...
    for (config = 0U; config < CDD_XBAR_ICL_XBAR_CONFIGURATIONS; config++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Output_Valid_Firstgroup(CddXbarConf_CddXbarIclXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_IclCfg[config].outputLine, CDD_XBAR_SID_INIT)) ||
...
static FUNC(void, CDD_XBAR_CODE) Cdd_Xbar_Icl_Priv(VAR(uint8, AUTOMATIC) config)
{
    ...
    for (input = 0U; input < Cdd_Xbar_Config->Cdd_Xbar_IclCfg[config].input_count; input++)
    {
        ...
        if ((E_OK != Cdd_Xbar_Type_Input_Valid_Firstgroup(CddXbarConf_CddXbarIclXbar, 
            &Cdd_Xbar_Config->Cdd_Xbar_IclCfg[config].inputLine[input], CDD_XBAR_SID_INIT)) ||
...
#if (0U < CDD_XBAR_INPUT_XBAR_CONFIGURATIONS)
/* Design: MCAL-28371 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_InSelect_Priv
...
VAR(uint16, AUTOMATIC) InputLine, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    { 
        if(CDD_XBAR_OUTPUT_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_OutSelect(&Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine,
                &InputLine, Selection);
            retval = E_OK;
...
#if (0U < CDD_XBAR_EPWM_XBAR_CONFIGURATIONS)
/* Design: MCAL-28383 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) Cdd_Xbar_EpwmSelect_Priv
(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(uint16, AUTOMATIC) InputLine, VAR(boolean, AUTOMATIC) Selection)
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
Cdd_Xbar_OutOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_OUTPUT_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_OutOutputInvert(&Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine,
                &Selection);
            retval = E_OK;
        }
    }
...
Cdd_Xbar_EpwmOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_EPWM_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_EpwmOutputInvert(&Cdd_Xbar_Config->Cdd_Xbar_EpwmCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine,
                &Selection);
            retval = E_OK;
        }
    }
...
Cdd_Xbar_ClbOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_CLB_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_ClbOutputInvert(&Cdd_Xbar_Config->Cdd_Xbar_ClbCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine,
                &Selection);
            retval = E_OK;
        }
    }
...
Cdd_Xbar_MdlOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_MINDB_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_MdlOutputInvert(&Cdd_Xbar_Config->Cdd_Xbar_MindbCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine, 
                &Selection);
            retval = E_OK;
        }
    }
...
Cdd_Xbar_IclOutputInvert_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit, VAR(boolean, AUTOMATIC) Selection)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        if(CDD_XBAR_ICL_XBAR_CONFIGURATIONS > CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit))
        {
            Cdd_Xbar_IclOutputInvert(&Cdd_Xbar_Config->Cdd_Xbar_IclCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)].outputLine, 
                &Selection);
            retval = E_OK;
        }
    }
...
Cdd_Xbar_OutOutputInvertCheck_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            status = 
                Cdd_Xbar_OutOutputInvertCheck(&Cdd_Xbar_Config->Cdd_Xbar_OutputCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine);
        }
    }
    return status;
...
/* Design: MCAL-28385 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_EpwmOutputInvertCheck_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            status = 
                Cdd_Xbar_EpwmOutputInvertCheck(&Cdd_Xbar_Config->Cdd_Xbar_EpwmCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine);
        }
    }
    return status;
...
/* Design: MCAL-28392 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_ClbOutputInvertCheck_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            status = 
                Cdd_Xbar_ClbOutputInvertCheck(&Cdd_Xbar_Config->Cdd_Xbar_ClbCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine);
        }
    }
    return status;
...
/* Design: MCAL-28399 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_MdlOutputInvertCheck_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            status = 
                Cdd_Xbar_MdlOutputInvertCheck(&Cdd_Xbar_Config->Cdd_Xbar_MindbCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine);
        }
    }
    return status;
...
Cdd_Xbar_IclOutputInvertCheck_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    ...
    #endif  /* STD_ON == CDD_XBAR_DEV_ERROR_DETECT */
    {
        ...
        {
            status = 
                Cdd_Xbar_IclOutputInvertCheck(&Cdd_Xbar_Config->Cdd_Xbar_IclCfg[CDD_XBAR_GET_XBAR_INSTANCE(CrossbarUnit)]
                .outputLine);
        }
    }
    return status;
...
/* Design: MCAL-28372 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_InSelectLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit )
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28379 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_OutLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28386 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_EpwmLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit )
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28393 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_ClbLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit )
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28400 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_MdlLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit )
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28407 */
static FUNC(Std_ReturnType, CDD_XBAR_CODE) 
Cdd_Xbar_IclLock_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit )
{
    VAR(Std_ReturnType, AUTOMATIC)retval = E_NOT_OK;
...
/* Design: MCAL-28373 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_InSelectLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
...
/* Design: MCAL-28380 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_OutLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
...
/* Design: MCAL-28387 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_EpwmLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
...
/* Design: MCAL-28394 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_ClbLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
...
/* Design: MCAL-28401 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_MdlLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
...
/* Design: MCAL-28408 */
static FUNC(boolean, CDD_XBAR_CODE) 
Cdd_Xbar_IclLockStatus_Priv(VAR(Cdd_Xbar_Type, AUTOMATIC) CrossbarUnit)
{
    VAR(boolean, AUTOMATIC)status = FALSE;
