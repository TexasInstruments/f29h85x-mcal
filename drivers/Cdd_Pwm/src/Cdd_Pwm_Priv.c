 ...
 * \param[in]  Prescale   Prescale value for the PWM hardware instance. Prescale value is time Base
 *clock divider multiplied with high speed clock divider
 * \pre None
 * \post None
 * \return None
 * \retval None
...
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SetCounterCompareValue(uint32 Base, Cdd_Pwm_OuputChannelType OutputChannel, uint16 CompareCount);
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SetActionQualifierShadowLoadMode(uint32 Base, Cdd_Pwm_OuputChannelType Channel,
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SetTimeBaseCounterMode(uint32 Base, Cdd_Pwm_TimeBaseCountModeType CounterMode);
 ...
 *
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE) Cdd_Pwm_SetCompareShadowMode(uint32 Base, Cdd_Pwm_OuputChannelType OutputChannel,
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SetActionQualifierAction(uint32 Base, Cdd_Pwm_OuputChannelType ChannelType, Cdd_Pwm_AQOutputType Output,
                                     Cdd_Pwm_AQOutputEvtType Event);
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_ClearActionQualifierAction(uint32 Base, Cdd_Pwm_OuputChannelType ChannelType);
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SetInterruptSource(uint32 Base, Cdd_Pwm_InterruptSourceType InterruptSource);
 ...
 *********************************************************************************************************************/
LOCAL_INLINE FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_SingleActionQualifierAction(uint32 Base, Cdd_Pwm_OuputChannelType ChannelType, Cdd_Pwm_AQOutputType Output,
                                        Cdd_Pwm_AQOutputEvtType Event);
 ...
 *********************************************************************************************************************/
static FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_UpdateNotificationAction(VAR(Cdd_Pwm_ChannelType, AUTOMATIC) ChannelNumber,
 ...
 *********************************************************************************************************************/
static FUNC(void, CDD_PWM_CODE)
    Cdd_Pwm_UpdateNotification(VAR(Cdd_Pwm_ChannelType, AUTOMATIC) ChannelNumber,
...
LOCAL_INLINE FUNC(void, CDD_PWM_CODE) Cdd_Pwm_ConfigureInterrupt(uint32 Base, boolean Mode)
{
    ...
    HWREGH(Base + EPWM_O_ETSEL) =
        (HWREGH(Base + EPWM_O_ETSEL) & (~EPWM_ETSEL_INTEN)) | ((uint16)Mode * EPWM_ETSEL_INTEN);
}
...
FUNC(void, CDD_PWM_CODE) Cdd_Pwm_HwUnitInit(void)
{
    ...
    for (Cdd_Pwm_InstanceType hwunit_id = 0; hwunit_id < CDD_PWM_COUNT; hwunit_id++)
    {
        ...
        /* Clear zero & PRD match event as the interrupt trigger source since they are enabled by
         * default after reset*/
                           ...
                           VAR(Cdd_Pwm_EdgeNotificationType, AUTOMATIC) Notification, VAR(uint8, AUTOMATIC) IntEvtCount)
{
         ...
         channel_id++)
    {
        ...
        {
            /* Notification is enabled when the waveforn has neither 0 percent duty cycle nor 100
             * percent duty cycle */
...
FUNC(void, CDD_PWM_CODE) Cdd_Pwm_SetIdleOutput(VAR(Cdd_Pwm_ChannelType, AUTOMATIC) ChannelNumber)
{
    ...
    if (TRUE == Cdd_Pwm_DrvObjPtr->channelobj[ChannelNumber].notification_status)
    {
        ...
        Cdd_Pwm_DrvObjPtr->hwunitobj[hw_index].notification_active =
            (active & ~(1U << (uint8)(Cdd_Pwm_ConfigPtr->channelcfg[ChannelNumber].outputchannel)));
    }
#endif
                        ...
                        VAR(Cdd_Pwm_DutyCycleType, AUTOMATIC) DutyCycle)
{
    ...
    if ((0U == DutyCycle) || (0U == Cdd_Pwm_DrvObjPtr->hwunitobj[hw_index].period) || (0U == compare_value))
    {
        ...
        Cdd_Pwm_DrvObjPtr->hwunitobj[hw_index].notification_active =
            (active & ~(1U << (uint8)(Cdd_Pwm_ConfigPtr->channelcfg[ChannelNumber].outputchannel)));
#endif
        /* Set the AQ event to set the 0% duty cycle - This is effective when counter becomes equal
         * to Zero */
        Cdd_Pwm_SingleActionQualifierAction(base_addr, channel_cfg->outputchannel, output,
                                            CDD_PWM_AQ_OUTPUT_ON_TIMEBASE_ZERO);
    ...
    else if (CDD_PWM_MAX_DUTY_CYCLE == DutyCycle)
    {
        ...
        Cdd_Pwm_DrvObjPtr->hwunitobj[hw_index].notification_active =
            (active & ~(1U << (uint8)(Cdd_Pwm_ConfigPtr->channelcfg[ChannelNumber].outputchannel)));
#endif
        /* Set the AQ event to set the 100% duty cycle - This is effective when counter becomes
         * equal to Zero */
        Cdd_Pwm_SingleActionQualifierAction(base_addr, channel_cfg->outputchannel, output,
                                            CDD_PWM_AQ_OUTPUT_ON_TIMEBASE_ZERO);
        /* Update the channel output edge state -
...
#if (STD_ON == CDD_PWM_NOTIFICATION_SUPPORTED)
            notification_mode =
                ((TRUE == channelobj->notification_enable) && (FALSE == channelobj->notification_status));
...
#endif
            /* This setting is required for Symmetric waveform, when TBCTR = Zero then state should
             * be configured since the other two events are Compare Up & compare Down
             */
                                     ...
                                     VAR(boolean, AUTOMATIC) NotificationMode, VAR(boolean, AUTOMATIC) Mode)
{
    ...
    if (TRUE == NotificationMode)
    {
        if ((CDD_PWM_ASYMMETRIC_WAVEFORM == Cdd_Pwm_ConfigPtr->hwunitcfg[hw_index].symmetry) && (FALSE == Mode) &&
            (3U == Cdd_Pwm_DrvObjPtr->hwunitobj[hw_index].notification_active))
        {
             ...
             * then TBPRD event shouldn't be disabled  as the interrupt source since it is shared by
             * with channel */
        }
