...
/** \brief Cdd_Pwm_Notification_DetCheck - This API will report DET error for the notification
 *related APIs
 *
...
FUNC(void, CDD_PWM_CODE)
Cdd_Pwm_SetDutyCycle(VAR(Cdd_Pwm_ChannelType, AUTOMATIC) ChannelNumber, VAR(Cdd_Pwm_DutyCycleType, AUTOMATIC) DutyCycle)
{
#if (STD_ON == CDD_PWM_DEV_ERROR_DETECT)
    if (FALSE == Cdd_Pwm_IsInitialized)
    {
                                        ...
                                        VAR(uint8, AUTOMATIC) IntEvtCount)
{
    ...
    else if ((CDD_PWM_NONE <= Channel) ||
             (((Cdd_Pwm_CfgPtr->hwunitcfg[InstanceId].lastchannel - channel_id) == 0U) &&
              ((CDD_PWM_BOTH_A_AND_B == Channel) ||
               ((CDD_PWM_CHANNEL_A == Channel) &&
                ...
                (CDD_PWM_OUTPUT_A == Cdd_Pwm_CfgPtr->channelcfg[channel_id].outputchannel)))))
    {
         ...
         *  1) Requested notification channel type is CDD_PWM_NONE or greater than ID of the
         * CDD_PWM_NONE enum value 2) Requested notification channel type is CDD_PWM_BOTH_A_AND_B
         * but only one channel is configured for the PWM instance. 3) Requested notification
         * channel type is CDD_PWM_CHANNEL_A but only channel B is configured for the PWM instance.
                            ...
                            VAR(Cdd_Pwm_NotificationChannelType, AUTOMATIC) Channel)
{
    ...
    {
        /* This means either a det error is reported in the Cdd_Pwm_Notification_DetCheck function
         * or no det error is reported
         */
    }
...
#endif
    {
        ...
        {
            /* Notification should be disabled for the instance before enabling again for another
             * channel type */
            SchM_Enter_Cdd_Pwm_CDD_PWM_EXCLUSIVE_AREA_0();
        ...
        {
            /* Report Det runtime error that the notification for the channel should be disabled for
             * the channel which was enabled with EnableNotification API
             */
