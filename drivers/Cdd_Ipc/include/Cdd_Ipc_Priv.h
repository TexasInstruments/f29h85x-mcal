 ...
 *
 * \param[in] ChannelId : When Cdd Ipc is integrated with Autosar Com stack, this parameter is a Pdu identifier used
 *                        to address the respective channel. 
 *                        When Cdd Ipc is not integrated with Autosar Com stack, this parameter is a channel identifier 
 *                        used to address the respective channel.
...
(VAR(uint8 , AUTOMATIC) Instance_Index, P2VAR(uint8, AUTOMATIC, CDD_IPC_APPL_DATA) ChannelIndex);

/** \brief Copies the received data to the PduInfoPtr
 *
 * \param[in] Instance_Index is the index of the Rx instance in Cdd_Ipc_CfgPtr
 * \param[in] PduInfoPtr is the pointer to a struct of type PduInfoType where received data needs to be copied
 * \pre None
 * \post None
 * \return None
 *
 *********************************************************************************************************************/
FUNC(void, CDD_IPC_CODE) Cdd_Ipc_ReadChannelPriv
(VAR(uint8 , AUTOMATIC) Instance_Index, P2CONST(PduInfoType, AUTOMATIC, CDD_IPC_APPL_CONST) PduInfoPtr);
 ...
 *
 * \param[in] ChannelId : Symbolic name of the Tx Pdu to be transmitted when Cdd_Ipc is integrated with Com Stack.
 *                        Symbolic name Tx channel when Cdd_Ipc is not integrated with Com Stack.
 * \param[out] ChannelIndex is pointer the channel index
 * \param[out] InstanceIndex is pointer the instance index
 * \pre None
 * \post None
