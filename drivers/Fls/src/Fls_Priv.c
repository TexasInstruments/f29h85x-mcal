...
#if (FLS_TIMEOUT_SUPERVISION_ENABLED == STD_ON)
static FUNC(Std_ReturnType, FLS_CODE) Fls_TimeoutVerification(Os_TickType startCount);
#endif
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_TimeoutVerification(Os_TickType startCount)
{
	VAR(Os_TickType, AUTOMATIC)tempCount = (Os_TickType)0U;
    VAR(Os_TickType, AUTOMATIC)elapsedCount = (Os_TickType)0U;
    VAR(StatusType, AUTOMATIC)status = E_NOT_OK;
    VAR(Std_ReturnType, AUTOMATIC)timeoutCheck = E_OK;
    
	VAR(uint32, AUTOMATIC)time1 = (uint32)0U;
	...
	switch(Fls_DrvObj.jobType) 
	{
		case FLS_JOB_ERASE: 
			...
			{
                /** We have max sector erase time, so we are multiplying it with CPU clock and also with 
                   number of sectors in one bank to get max number of cycles in order to compare with run 
                   time cycles 
                   FLS_MAX_ERASE_TIME: sector erase time in us
                   time1: clock cycles of sector erase  
                */
				time1 = FLS_MAX_ERASE_TIME * (FLS_CPU_CLOCK_FREQ / FLS_CONV_TO_MHZ);
			}
			else
			{	
                /** Based on F28H5x device datasheet, the bank erase takes 40% more than sector erase: 
                    1003ms vs 1410ms
                */
...
static FUNC(void, FLS_CODE) Fls_Process_JobCompare(uint32 chunkSize)
{
    ...
    else
    {
        ...
        Fls_PostProcessAndReportError((uint8)FLS_SID_COMPARE, failureType);
    }
}
...
static FUNC(void, FLS_CODE) Fls_Process_JobRead(uint32 chunkSize)
{
    ...
    /**  Post process and initiating next job or error flaging is done 
    ...
    */
    ...
    else
    {
        ...
        Fls_PostProcessAndReportError((uint8)FLS_SID_READ, failureType);
    }
}
...
static FUNC(void, FLS_CODE) Fls_Process_JobErase(uint32 chunkSize)
{
...
#if (FLS_TIMEOUT_SUPERVISION_ENABLED == STD_ON) 
        VAR(Os_TickType, AUTOMATIC)CurrentCount = 0U; 
	    (void)GetCounterValue(FLS_OS_COUNTER_ID, &CurrentCount);
#endif 
    ...
    */
    ...
    {
        Fls_PostProcessAndReportError((uint8)FLS_SID_ERASE, failureType);
    }
}
...
static FUNC(void, FLS_CODE)Fls_Process_JobWrite(uint32 chunkSize)
{
...
#if (FLS_TIMEOUT_SUPERVISION_ENABLED == STD_ON)
    counterStatus = GetCounterValue(FLS_OS_COUNTER_ID, &CurrentCount);
#endif
    ...
    */
    ...
    else
    {
        /** To release the Flash semaphore if erase operation failed*/
        Fls_SSU_releaseFlashSemaphore();
        Fls_PostProcessAndReportError((uint8)FLS_SID_WRITE, failureType);
    }
}
/**
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29Compare(uint32 actualChunkSize)
{
    ...
    VAR(uint32, AUTOMATIC)u32Length  = actualChunkSize / 4U; /** for 32-bit word compare, length should be /4 */
     ...
     *   P2CONST(uint8, AUTOMATIC, FLS_APPL_DATA) pu8CheckValueBuffer = (uint8 *) *Fls_DrvObj.ramAddr;
     *   P2CONST(uint8, AUTOMATIC, FLS_APPL_DATA) pu8StartAddress  = (uint8 *) Fls_DrvObj.flashAddr;
     *   oReturnCheck = Fls_Fapi_doVerifyByByte((uint8 *)pu8StartAddress, u32Length, 
     *                                 (uint8 *)pu8CheckValueBuffer, &oFlashStatusWord);
     */

    oReturnCheck = Fls_Fapi_doVerify((uint32*)pu32StartAddress, u32Length,
                                     (uint32*)pu32CheckValueBuffer, &oFlashStatusWord);
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29BlankCheck(uint32 actualChunkSize)
{
    ...
    oReturnCheck = Fls_Fapi_doBlankCheck((uint32*)pu32StartAddress,
                    u32Length,
                    &oFlashStatusWord); 
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29Read(uint32 actualChunkSize)
{
	P2VAR(uint8, AUTOMATIC, FLS_APPL_DATA)src = (uint8*)Fls_DrvObj.flashAddr; /**if read 32-bit word, use uint32* */
	P2VAR(uint8, AUTOMATIC, FLS_APPL_DATA)dest = (uint8*)Fls_DrvObj.ramAddr; /**if read 32-bit word, use uint32* */
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29AsyncWrite_sub(uint32 actualSize)
{
    ...
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_STClrCMD;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_WriteCMD;
    ...
    if (Fls_U8FlashWriteStage == FLS_S_WRITE_PRECHECK)
    {
        ...
        {
            if (Fls_Fapi_checkFsmForReady() == FLS_FAPI_STATUS_FSMREADY)
            {
                pre_write_check = FLS_WRITE_FSM_ISSUE_CMD;
            }
            else
            {
                fsmStatus = MEMIF_BUSY_INTERNAL; /** Flash driver is busy with Internal management operations*/
        ...
        {
            if(oReturnCheck_STClrCMD == FLS_FAPI_STATUS_SUCCESS)
            {
                Fls_U8FlashWriteStage = FLS_DO_SECTOR_UNLOCK;
                pre_write_check = FLS_WRITE_FSM_READY_CHECK;
            }     
            else if(oReturnCheck_STClrCMD == FLS_FAPI_STATUS_FSMBUSY)
            {
                fsmStatus = MEMIF_BUSY_INTERNAL;
            }      
    ...
    if(Fls_U8FlashWriteStage == FLS_DO_WRITE_JOB )
    {
       /** DataBufSizeInBytes can be 8 or 16. ALl other values are not allowed */
        oReturnCheck_WriteCMD = Fls_Fapi_issueProgrammingCommand((uint32 *)u32StartAddress, 
                                                                 (uint8 *) buf, 
                                                                 (uint8) DataBufSizeInBytes); 
    ...
    if (Fls_U8FlashWriteStage == FLS_S_WRITE_POSTCHECK)
    {
        if( post_write_check == FLS_WRITE_FSM_READY_CHECK)
        {
            ...
            if (oReturnCheck != FLS_FAPI_STATUS_FSMBUSY)
            {
        ...
        {
            if(oReturnCheck_WriteCMD == FLS_FAPI_STATUS_SUCCESS)
            {
                post_write_check = FLS_WRITE_FSM_STATUS_CHECK;
            }     
            else if(oReturnCheck_WriteCMD == FLS_FAPI_STATUS_FSMBUSY)
            {
                fsmStatus = MEMIF_BUSY_INTERNAL;
            }      
            else  /** FLS_FAPI_ERROR_INVALIDADDRESS, etc*/
...
{
    VAR(Std_ReturnType, AUTOMATIC)retVal = E_OK;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheckA;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheckB;
    ...
    if((oReturnCheckA == FLS_FAPI_STATUS_SUCCESS) && (oReturnCheckB == FLS_FAPI_STATUS_SUCCESS)){
        retVal = E_OK;
    }
    else
    {
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29AsyncSectorErase_sub(uint32 actualSize)
{
    ...
    VAR(Fls_FapiFlashStatus, AUTOMATIC) oFlashStatus;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_AsyncCMD;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_EraseCMD;
    VAR(uint32, AUTOMATIC)u32StartAddress = Fls_DrvObj.flashAddr;
    VAR(uint32, AUTOMATIC)length = Fls_DrvObj.length;
    ...
    if (Fls_U8SectorEraseStage == FLS_S_ERASE_PRECHECK ) /** FLS_S_ERASE_PRECHECK=1*/
    {
        if (pre_check_sub_function == FLS_ERASE_FSM_READY_CHECK)
        {
            ...
            if (oReturnCheck == FLS_FAPI_STATUS_FSMREADY)
            {
        ...
        {
            if(oReturnCheck_AsyncCMD == FLS_FAPI_STATUS_SUCCESS)
            {
                Fls_U8SectorEraseStage = FLS_DO_SECTOR_UNLOCK;
                pre_check_sub_function = FLS_ERASE_FSM_READY_CHECK;
            }     
            else if(oReturnCheck_AsyncCMD == FLS_FAPI_STATUS_FSMBUSY)
            {
                fsmStatus = MEMIF_BUSY_INTERNAL;
            }      
            else  /** FLS_FAPI_ERROR_INVALIDADDRESS, etc*/
    ...
    if (Fls_U8SectorEraseStage == FLS_S_ERASE_POSTCHECK)
    {
        ...
        {
            if(oReturnCheck_EraseCMD == FLS_FAPI_STATUS_SUCCESS)
            {
                post_check_sub_function = FLS_ERASE_FSM_STATUS_CHECK;
            }     
            else if(oReturnCheck_EraseCMD == FLS_FAPI_STATUS_FSMBUSY)
            {
                fsmStatus = MEMIF_BUSY_INTERNAL; /** MEMIF_BUSY_INTERNAL=0x3*/
            }      
            else  /** FLS_FAPI_ERROR_INVALIDADDRESS, etc*/
            {
                retVal = E_NOT_OK;
...
static FUNC(Std_ReturnType, FLS_CODE) Fls_F29AsyncBankErase_sub(uint32 actualSize)
{
    ...
    VAR(MemIf_StatusType, AUTOMATIC)fsmStatus = MEMIF_BUSY;
    VAR(Fls_FapiFlashStatus, AUTOMATIC) oFlashStatus;
    static VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_STClrCMD = FLS_FAPI_STATUS_SUCCESS;
    VAR(Fls_FapiStatusType, AUTOMATIC) oReturnCheck_BKEraseCMD = FLS_FAPI_STATUS_SUCCESS;
    ...
    if (Fls_U8BankEraseStage == FLS_S_ERASE_POSTCHECK)
    {
        ...
        {
            if(oReturnCheck_BKEraseCMD == FLS_FAPI_STATUS_SUCCESS)
            {
                post_bank_erase_check = FLS_ERASE_FSM_STATUS_CHECK;
            }     
            else if(oReturnCheck_BKEraseCMD == FLS_FAPI_STATUS_FSMBUSY)
            {
                fsmStatus = MEMIF_BUSY_INTERNAL;
            }      
            else  /** FLS_FAPI_ERROR_INVALIDADDRESS, etc*/
