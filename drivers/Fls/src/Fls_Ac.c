...
static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Write_32(uint32 ctrlBase, uint32 reg_offset, uint32 mask, 
...
static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_loopRegionForValue( uint32 *pu32StartAddress,  
                                                uint32 u32Length,
                                                Fls_FlashStatusWordType *poFlashStatusWord,
                                                uint32 *pu32CheckValue,
                                                Fls_FapiRegionValueType oRegionValue );

static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_loopRegionForValueByByte( uint8 *pu8StartAddress, 
                                                uint32 u32Length,
                                                Fls_FlashStatusWordType *poFlashStatusWord,
                                                uint8 *pu8CheckValue,
                                                Fls_FapiRegionValueType oRegionValue );

static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_checkRegionForValue( uint32 *pu32StartAddress, 
                                                uint32 u32Length,
                                                Fls_FlashStatusWordType *poFlashStatusWord,
                                                uint32 *pu32CheckValue,
                                                Fls_FapiRegionValueType oRegionValue );

static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_checkRegionForValueByByte( uint8 *pu8StartAddress, 
                                                uint32 u32Length,
                                                Fls_FlashStatusWordType *poFlashStatusWord,
                                                uint8 *pu8CheckValue,
                                                Fls_FapiRegionValueType oRegionValue );
...
/**
 *   This is the primary function to initialize the API for first use or when the user changes active 
 *   Flash Wrappers or System Frequency
 */
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_initializeAPI( uint32 u32HclkFrequency  )
{
    ...
    {
        oErrorReturn = Fls_Write_32(FLS_FLASH_READINTERFACE_BASE, FLS_FLASH_O_FLCLKCTL, 
                                FLS_FLASH_FLCLKCTL_KDIV_M, FLS_FLASH_FLCLKCTL_KDIV_S, 2U);
        oErrorReturn_1 = Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CFGCMD, 
                                    FLS_FLASH_NW_CFGCMD_WAITSTATE_M, 
    ...
    {
        oErrorReturn = Fls_Write_32(FLS_FLASH_READINTERFACE_BASE, FLS_FLASH_O_FLCLKCTL, 
                                FLS_FLASH_FLCLKCTL_KDIV_M, FLS_FLASH_FLCLKCTL_KDIV_S, 1U);
        oErrorReturn_1 = Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CFGCMD, 
                                    FLS_FLASH_NW_CFGCMD_WAITSTATE_M, 
                                    FLS_FLASH_NW_CFGCMD_WAITSTATE_S, 2U);
    }
    ...
    {
        oErrorReturn = Fls_Write_32(FLS_FLASH_READINTERFACE_BASE, FLS_FLASH_O_FLCLKCTL, 
                                FLS_FLASH_FLCLKCTL_KDIV_M, FLS_FLASH_FLCLKCTL_KDIV_S, 0U);
        oErrorReturn_1 = Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CFGCMD, 
                                    FLS_FLASH_NW_CFGCMD_WAITSTATE_M, 
                                    FLS_FLASH_NW_CFGCMD_WAITSTATE_S, 2U);
    }
...
*/
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_issueProgrammingCommand( uint32 *pu32StartAddress,
                                              uint8  *pu8DataBuffer,
                                              uint8  u8DataBufferSizeInBytes
                                            )
{
    VAR(Fls_FapiStatusType, AUTOMATIC) oErrorReturn = FLS_FAPI_STATUS_SUCCESS;
    /*VAR(Fls_Fapi_FlashProgrammingCommandsType, AUTOMATIC) oCurrentMode = FLS_Fapi_AutoEccGeneration;*/
    ...
    P2VAR(uint32, AUTOMATIC, FLS_APPL_DATA) pu32DataBuffer = (uint32*)pu8DataBuffer; /**QJ, problem */
    ...
    if( oErrorReturn == FLS_FAPI_STATUS_SUCCESS )
    {
        ...
        /*LDRA_INSPECTED 439 S MR:R.11.2,R.11.4,R.11.6,R.11.7 "Reason - Cast from pointer to integral type required"*/
    ...
    if( oErrorReturn == FLS_FAPI_STATUS_SUCCESS )
    {
        ...
        (void) Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CMDCTL, 
                        FLS_FLASH_NW_CMDCTL_ECCGENOVR, FLS_FLASH_NW_CMDCTL_ECCGENOVR_S, 0U);

        /** Enable DATAVEREN to catch 0->1 program request */
        (void) Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CMDCTL, 
                        FLS_FLASH_NW_CMDCTL_DATAVEREN, FLS_FLASH_NW_CMDCTL_DATAVEREN_S, 1U);

        /** Configure for 1x128 word program: Command is program (1), Size is ONEWORD (0) */
        HWREG(FLS_FLASH_FAPI_FLASHNW_FC1_BASE + FLS_FLASH_NW_O_CMDTYPE) = 
                    FLS_FLASH_NW_CMDTYPE_PROG_ONEWD;

        /** Read back for FLS_FLASH_NW_O_CMDTYPE register */
        oErrorReturn = Fls_Fapi_regReadback((uint32)FLS_FLASH_NW_O_CMDTYPE, 
                                            (uint32) FLS_FLASH_NW_CMDTYPE_PROG_ONEWD);
                ...
                if((u32Index % 4U) == 0U)
                {
                    if(((u32Index == 0U) || (u32Index == 4U)))
                    {
                        (void) Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CMDBYTEN, 
                                        ...
                                        FLS_FLASH_NW_CMDBYTEN_ECC0VAL_S, 1U);
                    }
                    else
                    {
                        (void) Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CMDBYTEN, 
                                        FLS_FLASH_NW_CMDBYTEN_ECC1VAL_M, 
                                        FLS_FLASH_NW_CMDBYTEN_ECC1VAL_S, 1U);
                    }

                    HWREG(FLS_FLASH_FAPI_FLASHNW_FC1_BASE + FLS_FLASH_NW_O_CMDDATA0 + (u32Index)) 
                                                   = *pu32DataBuffer;

                    pu32DataBuffer++;

                    HWREG(FLS_FLASH_FAPI_FLASHNW_FC1_BASE + FLS_FLASH_NW_O_CMDBYTEN) |= 
                                (((uint32)0xF) << (u32Index));
                }
            }
        ...
        /* Disable AUTOCALC */
        (void) Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, FLS_FLASH_NW_O_CMDCTL, 
                          FLS_FLASH_NW_CMDCTL_ECCGENOVR, FLS_FLASH_NW_CMDCTL_ECCGENOVR_S, 1U);
    }
 ...
 */
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_issueAsyncCommandWithAddress( Fls_FlashStateCmdType oCommand,
                                                 ...
                                                 )
{
    ...
    if(oErrorReturn == FLS_FAPI_STATUS_SUCCESS)
    {
        ...
        if( Fls_Fapi_regReadback((uint32)FLS_FLASH_NW_O_CMDTYPE, 
                                    (uint32) FLS_FLASH_NW_CMDTYPE_SECTOR_ERASE) == FLS_FAPI_STATUS_SUCCESS)
        {
...
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_issueBankEraseCommand( uint32 *pu32StartAddress )
{
    ...
    if( oErrorReturn == FLS_FAPI_STATUS_SUCCESS )
    {
        ...
        /**Readback FLS_FLASH_O_CMDTYPE register */
        if( Fls_Fapi_regReadback((uint32)FLS_FLASH_NW_O_CMDTYPE, 
                                 (uint32)FLS_FLASH_NW_CMDTYPE_BANK_ERASE) == FLS_FAPI_STATUS_SUCCESS)
        {
        ...
        if(oErrorReturn == FLS_FAPI_STATUS_SUCCESS)
        {
             ...
             */
            oErrorReturn = Fls_Write_32((uint32)FLS_FLASH_FAPI_FLASHNW_FC1_BASE, 
...
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_issueAsyncCommand(Fls_FlashStateCmdType oCommand)
{
    ...
    {	
        ...
        {   
            ...
            oErrorReturn = Fls_Write_32(FLS_FLASH_FAPI_FLASHNW_FC1_BASE, 
 ...
 */
static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_checkRegionForValue(
                                                uint32 *pu32StartAddress,
                                                uint32 u32Length,
                                                Fls_FlashStatusWordType *poFlashStatusWord,
                                                uint32 *pu32CheckValue,
                                                ...
                                                )
{
    ...
    {
        /*LDRA_INSPECTED 488 S MR:R.10.3,R.10.4 "Reason - LDRA tool issue. Value is within the range"*/
        (void)Fls_Fapi_flushPipeline();
        if( Fls_Fapi_loopRegionForValue(pu32StartAddress, u32Length, 
                                        poFlashStatusWord, pu32CheckValue,
                                                ...
                                                )
{
    ...
    if(Fls_Fapi_isAddressValid((uint32)pu8StartAddress, u32Length/4U) == (boolean)TRUE)
    {
        /*LDRA_INSPECTED 488 S MR:R.10.3,R.10.4 "Reason - LDRA tool issue. Value is within the range"*/
        (void)Fls_Fapi_flushPipeline();
        if( Fls_Fapi_loopRegionForValueByByte(pu8StartAddress,u32Length, poFlashStatusWord, 
                                                pu8CheckValue, oRegionValue) != FLS_FAPI_STATUS_SUCCESS )
        {
            /** save read mode */
            poFlashStatusWord->au32StatusWord[3] = 0U;
            oErrorReturn =FLS_FAPI_ERROR_FAIL;
 ...
 */
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_doVerify(  uint32 *pu32StartAddress,
                                uint32 u32Length,
...
{  
    VAR(Fls_FapiStatusType, AUTOMATIC) oErrorReturn = FLS_FAPI_STATUS_SUCCESS;
    oErrorReturn = Fls_Fapi_checkRegionForValue(
                                            pu32StartAddress,
                                            u32Length,
                                            poFlashStatusWord,
...
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_doVerifyByByte( uint8 *pu8StartAddress,
                                    uint32 u32Length,
                                   ...
                                   )
{
    ...
    return( Fls_Fapi_checkRegionForValueByByte(
                                    pu8StartAddress,
                                    u32Length,
                                    poFlashStatusWord,
                                    pu8CheckValueBuffer,
 ...
 */
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_doBlankCheck(
                                 uint32 *pu32StartAddress,
                                 uint32 u32Length,
                                 Fls_FlashStatusWordType *poFlashStatusWord
                                 )
...
static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_issueFsmCommand(Fls_FlashStateCmdType oCommand)
{
    /**Issue the command only if the command is valid */
    if((FLS_FAPI_CLEARSTATUS   == oCommand) ||
        (FLS_FAPI_PROGRAMDATA   == oCommand) ||
        (FLS_FAPI_ERASEBANK    == oCommand) ||
...
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_setupBankSectorEnable(uint32 reg_address, uint32 value)
{
    ...
    if( (reg_address == (FLS_FLASH_NW_O_CMDWEPROTA))    ||
        (reg_address == (FLS_FLASH_NW_O_CMDWEPROTB))    ||
...
FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Fapi_checkFsmForReady(void)
{
    ...
    if((HWREG(FLS_FLASH_FAPI_FLASHNW_FC1_BASE + FLS_FLASH_NW_O_STATCMD) & 
                      FLS_FLASH_NW_STATCMD_CMDINPROGRESS) == 0U)
    {
...
static FUNC(boolean, FLS_CODE) Fls_Fapi_isAddressValid(uint32 u32Address, uint32 u32Length)
{
    ...
    if( 
        ((u32Address >= FLS_C29FLASHBANKFR4RP0STARTADDRESS) && 
 ...
 */
static FUNC(Fls_FapiStatusType, FLS_CODE) Fls_Write_32(uint32 ctrlBase, uint32 reg_offset, uint32 mask, uint32 shift, uint32 value)
{
    ...
    return ((readback_value == HWREG(ctrlBase + reg_offset))? FLS_FAPI_STATUS_SUCCESS : FLS_FAPI_ERROR_FLASHREGSNOTWRITABLE);
} 
...
static FUNC(void, FLS_CODE) Fls_F29SetWaitstates(uint16 waitstates)
{
    ...
    if((waitstates <= 0xFU) && (waitstates >= 0x1U))
    {
        ...
        HWREG(FLS_FRI1_CTL_BASE + FLS_FRI_O_FRDCNTL) =  (HWREG(FLS_FRI1_CTL_BASE + FLS_FRI_O_FRDCNTL) & 
                                               ~(uint32)FLS_FRI_FRDCNTL_RWAIT_M) |
                                               ((uint32)waitstates << FLS_FRI_FRDCNTL_RWAIT_S);
    }
}

static FUNC(void, FLS_CODE) Fls_F29WriteTrims(uint32 reg_offset, uint32 mask,
                 uint32 shift, uint32 value)
{
    /** Set the requested bits to the value.*/
...
FUNC(void, FLS_CODE) Fls_Update_WaitStates(uint16 waitstates)
{
    ...
    Fls_F29ConfigFRI(FLS_FLASH_FRI4, FLS_FLASH_DATAPREREAD_DISABLE | 
    ...
    Fls_F29WriteTrims(FLS_ASYSCTL_O_FLASHTRIMCOMMIT, FLS_ASYSCTL_FLC1TRIMCOMMITREAD,
                     0, FLS_ASYSCTL_FLC1TRIMCOMMITREAD);
    Fls_F29WriteTrims(FLS_ASYSCTL_O_FLASHTRIMCOMMIT, FLS_ASYSCTL_FLC1TRIMCOMMITOTHER,
                     0, FLS_ASYSCTL_FLC1TRIMCOMMITOTHER);
    
    /** Enable data cache, code cache, prefetch, and data preread to improve 
     *  performance of code executed from flash
     */
    Fls_F29ConfigFRI(FLS_FLASH_FRI4, FLS_FLASH_DATAPREREAD_ENABLE | 
 ...
 * \brief return true if the semaphore is successfully assigned, false if semaphore is already assigned
