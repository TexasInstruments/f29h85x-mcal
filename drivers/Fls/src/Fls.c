...
#if ((FLS_CFG_MAJOR_VERSION != (1U)) || \
     (FLS_CFG_MINOR_VERSION != (0U)) || \
     (FLS_CFG_PATCH_VERSION != (0U)))
...
/** SWS_Fls_00015*/
    ...
    else if ( (CfgPtr->FlsMaxWriteNormalMode == (uint32)0U) ||
                           ((CfgPtr->FlsMaxWriteNormalMode % (uint32)8U) != 0U) ||
...
#endif /** #if (STD_ON == FLS_DEV_ERROR_DETECT) */
    {
        ...
        if(FLS_CPU_CLOCK_FREQ >= 150000000U) {
            waitStates = 3U;
        }else if (FLS_CPU_CLOCK_FREQ >= 100000000U){
            waitStates = 2U;
        }else{
            waitStates = 1U;
        }
        ...
        **/
        ...
        if (Fls_DrvObj.FlsMaxWriteNormalMode > 16U)
        {
...
FUNC(Std_ReturnType, FLS_CODE) Fls_Erase(Fls_AddressType TargetAddress, Fls_LengthType Length)
{
    ...
    /** Fls_Erase erases a flash memory block starting from FlsBaseAddress + TargetAddress of size Length.
      Length should be rounded up to the next full sector boundary since only complete flash sectors 
      can be erased.
    */
    VAR(Fls_AddressType, AUTOMATIC)eraseStartAddress = (TargetAddress & 0xFFFFFU) + (FLS_BASE_ADDRESS & FLS_BASE_ADDRESS_REQ);

#if (STD_ON == FLS_DEV_ERROR_DETECT)
    /** SWS_Fls_00065*/
    /** If development error detection is enabled: the Fls_Erase shall check that the FLS module has been 
        initialized. If this check fails, the Fls_Erase shall reject the erase request, raise the development
        error FLS_E_UNINIT and return with E_NOT_OK
    */
    ...
    /** SWS_Fls_00023*/
    /** If development error detection is enabled: the Fls_Erase shall check that the FLS module is currently 
        not busy. If this check fails, the Fls_Erase shall reject the erase request, raise the development 
        error FLS_E_BUSY and return with E_NOT_OK
    */   
    ...
    /** SWS_Fls_00020*/ 
    /** SRS_Fls_12159*/
    /** If development error detection is enabled: the Fls_Erase shall check that the erase start address 
      (FlsBaseAddress + TargetAddress) is aligned to a flash sector boundary and that it lies within the specified 
      lower and upper flash address boundaries. If this check fails, the Fls_Erase shall reject the erase request, 
      raise the development error FLS_E_PARAM_ADDRESS and return with E_NOT_OK
    */
    ...
    /** SWS_Fls_00021*/
    /** SRS_Fls_12159*/
    /** If development error detection for is enabled: the Fls_Erase shall check that the erase length is 
        greater than 0 and that the erase end address (erase start address + length) is aligned to a flash 
        sector boundary and that it lies within the specified upper flash address boundary. If this check 
        fails, the Fls_Erase shall reject the erase request, raise the development error FLS_E_PARAM_LENGTH 
                                        ...
                                        Fls_LengthType Length)
{
    ...
    */
    VAR(Fls_AddressType, AUTOMATIC)ReadStartAddress = (SourceAddress & 0xFFFFFU) + (FLS_BASE_ADDRESS & FLS_BASE_ADDRESS_REQ);
#if (STD_ON == FLS_DEV_ERROR_DETECT)
    /** SWS_Fls_00099*/
    ...
    */
    ...
    /** If development error detection for the module Fls is enabled: the function Fls_Read shall check 
      that the driver is currently not busy. If this check fails, the function Fls_Read shall reject 
      the read request, raise the development error FLS_E_BUSY and return with E_NOT_OK
    */
    else if (MEMIF_BUSY == Fls_DrvObj.status)
    {
    ...
    /** SWS_Fls_00158*/
    /** If development error detection for the module Fls is enabled: the function Fls_Read shall check 
      the given data buffer pointer for not being a null pointer. If the data buffer pointer is a null pointer, 
      the function Fls_Read shall reject the read request, raise the development error FLS_E_PARAM_DATA 
      and return with E_NOT_OK
    */
    ...
    /** SWS_Fls_00097*/
    /** If development error detection for the module Fls is enabled: the function Fls_Read shall check that 
      the read start address (FlsBaseAddress + SourceAddress) lies within the specified lower and upper 
      flash address boundaries. If this check fails, the function Fls_Read shall reject the read job, 
      raise development error FLS_E_PARAM_ADDRESS and return with E_NOT_OK
    */
    ...
    /** SWS_Fls_00098*/
    /** If development error detection for the module Fls is enabled: the function Fls_Read shall check that 
      the read length is greater than 0 and that the read end address (read start address + length) lies within 
      the specified upper flash address boundary. If this check fails, the function Fls_Read shall reject the 
      read job, raise the development error FLS_E_PARAM_LENGTH and return with E_NOT_OK
    */
                                         ...
                                         Fls_LengthType Length)
{
    ...
    */
    ...
    VAR(Fls_AddressType, AUTOMATIC) writeStartAddress = (TargetAddress & 0xFFFFFU) + (FLS_BASE_ADDRESS & FLS_BASE_ADDRESS_REQ);
    ...
    */
    ...
    /** SWS_Fls_00030*/
    /** If development error detection for the module Fls is enabled: the function Fls_Write shall check that 
      the FLS module is currently not busy. If this check fails, the function Fls_Write shall reject the write 
    ...
    */
    ...
    /** If development error detection for the module Fls is enabled: the function Fls_Write shall 
      ...
      is a null pointer, the function Fls_Write shall reject the write request, raise the development 
    ...
    */
    ...
    /** SWS_Fls_00026*/
    /** SRS_Fls_12159*/
    /** If development error detection for the module Fls is enabled: the function Fls_Write shall check 
        that the write start address (FlsBaseAddress + TargetAddress) is aligned to a flash page boundary 
        and that it lies within the specified lower and upper flash address boundaries. If this check 
        fails, the function Fls_Write shall reject the write request, raise the development error 
    ...
    */
    ...
    /** SRS_Fls_12159*/
    /** If development error detection for the module Fls is enabled: the function Fls_Write shall check 
        that the write length is greater than 0, that the write end address (write start address + length) 
        is aligned to a flash page boundary and that it lies within the specified upper flash address 
        boundary. If this check fails, the function Fls_Write shall reject the write request, raise the 
                            ...
                            Fls_LengthType Length)
{
    ...
    */
    ...
    VAR(Fls_AddressType, AUTOMATIC)compareStartAddress = (SourceAddress & 0xFFFFFU) + (FLS_BASE_ADDRESS & FLS_BASE_ADDRESS_REQ);
    
#if (STD_ON == FLS_DEV_ERROR_DETECT)

    /** SWS_Fls_00152*/
    /** If development error detection for the module Fls is enabled: the function Fls_Compare 
      shall check that the driver has been initialized. If this check fails, the function Fls_Compare 
      shall reject the compare job, raise the development error FLS_E_UNINIT and return with E_NOT_OK
    */
    if (MEMIF_UNINIT == Fls_DrvObj.status)
    {
...
FUNC(Std_ReturnType, FLS_CODE) Fls_BlankCheck( Fls_AddressType TargetAddress, Fls_LengthType Length )
{
    ...
    */
    ...
    VAR(Fls_AddressType, AUTOMATIC)BlankCheckStartAddress = (TargetAddress & 0xFFFFFU) + (FLS_BASE_ADDRESS & FLS_BASE_ADDRESS_REQ);
    ...
    */
    ...
    /** If development error detection for the module FLS is enabled: the function Fls_BlankCheck shall 
      check that the driver is currently not busy. If this check fails, the function Fls_BlankCheck 
      shall reject the verification request, raise the development error FLS_E_BUSY and return with 
      E_NOT_OK
    */
    else if (MEMIF_BUSY == Fls_DrvObj.status)
    {
    ...
    /** SWS_Fls_00380*/
    /** If development error detection for the module FLS is enabled; the function Fls_BlankCheck shall check 
      that the verification start address (FlsBaseAddress + TargetAddress) lies within the specified lower 
      and upper flash address boundaries. If this check fails, the function Fls_BlankCheck shall reject the 
      verification job, raise the development error FLS_E_PARAM_ADDRESS and return with E_NOT_OK
    */
    ...
    /** SWS_Fls_00381*/
    /** If development error detection for the module FLS is enabled: the function Fls_BlankCheck shall check that 
      the given length is greater than 0 and that the verification end address (verification start address + length) 
      lies within the specified upper flash address boundary. If this check fails, the function Fls_BlankCheck shall 
      reject the verification job, raise the development error FLS_E_PARAM_LENGTH and return with E_NOT_OK
    */
...
FUNC(Std_ReturnType, FLS_CODE) Fls_GetVersionInfo( P2VAR(Std_VersionInfoType, AUTOMATIC, FLS_APPL_DATA) versioninfo )
{
    VAR(Std_ReturnType, AUTOMATIC)retVal = (Std_ReturnType) E_OK;
    /** SWS_Fls_00363*/
    ...
    */
        ...
        if (NULL_PTR == versioninfo)
        {
            (void) Det_ReportError(FLS_MODULE_ID, FLS_INSTANCE_ID, 
                                    FLS_SID_GET_VERSION_INFO, FLS_E_PARAM_POINTER);
            retVal = (Std_ReturnType) E_NOT_OK;
        }
        else
    #endif  /**  #if (STD_ON == FLS_DEV_ERROR_DETECT) */
...
FUNC(void, FLS_CODE) Fls_Cancel(void)
{
	#if (STD_ON == FLS_DEV_ERROR_DETECT)
    /** SWS_Fls_00356*/
    /** If development error detection for the module Fls is enabled: the function Fls_Cancel shall check that 
      the FLS module has been initialized. If this check fails, the function Fls_Cancel shall raise the 
      development error FLS_E_UNINIT and return
    */
	if (MEMIF_UNINIT == Fls_DrvObj.status)
	{
	...
	#endif /**  #if (STD_ON == FLS_DEV_ERROR_DETECT) */
	{
        ...
        */
        ...
        /** SWS_Fls_00147*/
        /** If configured, the function Fls_Cancel shall call the error notification function to inform the 
          caller about the cancellation of a job*/
...
static Std_ReturnType Fls_CheckValidAddress( Fls_AddressType SourceAddress )
{
    ...
    if((SourceAddress >= startAddr) &&
       (SourceAddress < (startAddr + (Fls_AddressType)FLS_TOTAL_SIZE)))
    {
        retVal = E_OK;
    }
    else
...
/**
 * Check if address for write/blank-check/compare is aligned with maximum operatable size  (main and non-main range)
 */
