 ...
 */
#if ((LIN_AR_RELEASE_MAJOR_VERSION != (0x04U)) || \
     (LIN_AR_RELEASE_MINOR_VERSION != (0x03U)) || \
 ...
 * 
 * \param[in] Channel LIN channel to be addressed.
 * \param[out] Lin_SduPtr Pointer to pointer to a shadow buffer or memory mapped LIN Hardware receive buffer where 
 *                        the current SDU is stored.
 * \pre None
 * \post None
 * \return None
 * \retval None
...
Lin_Init(P2CONST(Lin_ConfigType, AUTOMATIC, LIN_CONFIG_DATA) Config)
{
    ...
    if (NULL_PTR == Config)
    {
        /* Assigning passed module configuration to local Config pointer incase of Pre Compile variant */
    ...
    if (NULL_PTR != Config)
    {
        /* Assigning passed module configuration to local Config pointer incase of Post Build variant */
        config_ptr = Config;
    }
