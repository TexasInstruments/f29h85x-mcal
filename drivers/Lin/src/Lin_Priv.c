...
/** \brief Lin_ServiceInterrupts - This API will Service Lin interrupts for selected Interrupt line and will 
 ...
 * Design: MCAL-25669
 */
FUNC(Std_ReturnType, LIN_CODE) Lin_InitInternal(P2CONST(Lin_ConfigType, AUTOMATIC, LIN_CONFIG_DATA) Lin_ConfigPtr)
{
...
{	
    VAR(Std_ReturnType, AUTOMATIC) return_value = (Std_ReturnType)E_NOT_OK;
    VAR(uint32, AUTOMATIC) base = Lin_Drv_Config_Ptr->linChannelCfg[Channel].linControllerConfig.CntrAddr;
 ...
 * Design: MCAL-25678
 */
FUNC(void, LIN_CODE) Lin_GetData(uint32 channelID, uint32 base, \
P2VAR(uint8*, AUTOMATIC, LIN_APPL_CONST) sduPtr)
{
