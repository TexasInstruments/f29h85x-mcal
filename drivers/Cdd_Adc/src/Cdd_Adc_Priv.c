...
FUNC(void, CDD_ADC_CODE) Cdd_Adc_HwUnitInit(void)
{
    ...
    /* only one delay is necessary as long as the delay occurs after all the ADCs have begun
     * powering up */
    if (internal_vref == 0U)
    {
...
FUNC(void, CDD_ADC_CODE) Cdd_Adc_HwUnitDeinit(void)
{
    ...
    for (group_id = 0U; group_id < CDD_ADC_GROUP_CNT; group_id++)
    {
        ...
        {
            /* Nothing to be done if the group conversion has already stopped before calling DeInit
             * API. */
        }
        else
        {
...
FUNC(void, CDD_ADC_CODE) Cdd_Adc_StopGroup(Cdd_Adc_GroupType Group)
{
    ...
    if ((groupcfg.trigsrc_type == CDD_ADC_TRIGG_SRC_HW) &&
        (((Cdd_Adc_DrvObjPtr->group_obj[Group].implicit_stop == TRUE) &&
          (Cdd_Adc_DrvObjPtr->group_obj[Group].valid_samples ==
           Cdd_Adc_ConfigPtr->groupcfg[Group].stream_numsamples)) ||
         (Cdd_Adc_DrvObjPtr->group_obj[Group].grp_status == CDD_ADC_IDLE)))
    {
...
static FUNC(void, CDD_ADC_CODE) Cdd_Adc_ConfigPpbUnit(Cdd_Adc_PpbType PpbId)
{
    ...
    Cdd_Adc_PpbOffsetCorrection(base_addr, ppbid, Cdd_Adc_ConfigPtr->ppbcfg[PpbId].ppb_caloffset);
    /* Set the reference offset for the hardware unit */
    Cdd_Adc_SetPpbRefOffset(base_addr, ppbid, Cdd_Adc_ConfigPtr->ppbcfg[PpbId].ppb_refoffset);
    /* PPB result configuration */
    Cdd_Adc_ConfigurePpbResult(
        base_addr, ppbid, (Cdd_Adc_ConfigPtr->ppbcfg[PpbId].ppbresconfig | Cdd_Adc_ConfigPtr->ppbcfg[PpbId].soc_num));
...
static FUNC(void, CDD_ADC_CODE) Cdd_Adc_StopHwGrpTrigger(Cdd_Adc_GroupType Group)
{
         ...
         channelnum++)
    {
        ...
        Cdd_Adc_SetupSoc(base_addr, Cdd_Adc_ConfigPtr->channelcfg[channelnum].soc_num, CDD_ADC_TRIGGER_SW_ONLY,
                         Cdd_Adc_ConfigPtr->channelcfg[channelnum].channel_id,
                         Cdd_Adc_ConfigPtr->channelcfg[channelnum].sample_window);
...
static FUNC(void, CDD_ADC_CODE) Cdd_Adc_ProcessGroup(Cdd_Adc_GroupType Group)
{
    ...
    {
        /* Don't perform any action if the buffer is completely filled for implictily stopped groups
         * or stopped hardware groups. ISR may be called even after the buffer is complete when
         * continuetointerrupt mode is enabled and the hardware triggers are really fast
         */
    }
    else
    {
