...
typedef enum
{
    ...
    CDD_ADC_BUSY,              /*!< \brief  The conversion of the specified group has been started and is still going on */
    CDD_ADC_COMPLETED,         /*!< \brief  A conversion round (not the final one) of the group has been finished.*/
...
typedef enum
{
    /** \brief The ADC Driver stops the conversion as soon as the stream buffer is full (number of samples reached) */
...
typedef enum
{
    /** \brief Exactly one conversion of each channel in an ADC channel group is performed after the trigger event */
...
typedef struct Cdd_Adc_PpbUnitCfgTag
{
    ...
    /** \brief  Determines delta enable, two's complement, absolute enable, cycle by cycle enable functionality */
    uint16 ppbresconfig;
...
typedef struct Cdd_Adc_GroupCfgTag
{
    ...
    /** \brief  Boolean to detetrmine if the interrupt is configured as a trigger source for the group */
...
typedef struct Cdd_Adc_HwUnitCfgTag
{
    ...
    /** \brief  Adc hardwrae unit Soc priority number. Based on this RoundRobin, high priority are decided */
...
{
    /** \brief  Bitset to determine result1 overflow triggers interrupt to the CPU or events signals to EPWM/OUTPUT-XBAR */
    uint16 res1ovf;
    /** \brief  Bitset to determine result2 overflow triggers interrupt to the CPU or events signals to EPWM/OUTPUT-XBAR  */
    uint16 res2ovf;
    /** \brief  Bitset to determine out-of-tolerance triggers interrupt to the CPU or events signals to EPWM/OUTPUT-XBAR */
    uint16 ootflg;
}Cdd_Adc_IntSrcType;
...
typedef struct Cdd_Adc_CheckerResCfgTag
{
    ...
    uint32 base_addr;
    /** \brief  The two results can be from the same or different ADC HW Units whose result is to be compared */
...
{
    /** \brief  HwIndex mask to enable global sw trigger (with respect to the index of the hardware units)*/
    uint8 hwunit_mask;
...
/** \brief This service initializes ADC driver with the group specific result buffer start address where the 
 * conversion results will be stored.
 *
 *  This function initializes the group specific result buffer start address with the passed DataBufferPtr parameter
 *  where the conversion results will be stored.
 *  Note: The application has to ensure that the application buffer, where DataBufferPtr points to, can hold all the 
 *  conversion results of the specified group. The initialization with Cdd_Adc_SetupResultBuffer is required after 
 ...
 *              CDD_ADC_BUSY:             The conversion of the specified group has been started and is still going on 
 *              CDD_ADC_COMPLETED:        A conversion round of the specified group has been finished.  
 ...
 *
 * This service reads the group conversion result of the last completed conversion round of the requested group and 
 * stores the channel values starting at the DataBufferPtr address. The group channel values are stored in ascending 
 * channel number order (in contrast to the storage layout of the result buffer ifstreaming access is configured).
 *
 * \param[in] Group     Numeric ID of requested ADC Channel group.
 * \param[out] DataBufferPtr  Results of all channels of the group are stored where this pointer points to.
 * \pre None
 * \post None
 ...
 *
 * This service returns the number of valid samples per channel, stored in the result buffer. Reads a pointer, pointing
 * to a position in the group result buffer. With the pointer position, the results of all groupchannels of the last 
 * completed conversion round can be accessed .With the pointer and the return value, all valid group conversion 
 * results can be accessed (the user has to take the layout of the result buffer into account).
 *
 * \param[in] Group     Numeric ID of requested ADC Channel group.
 ...
 *
 * This service returns the safety checker status and clears the interrupt flags. This API needs to be called 
 * inside ESM ISR. This shouldn't be called directly.
 *
 ...
 *
 *********************************************************************************************************************/
FUNC(void,CDD_ADC_CODE) Cdd_Adc_ReadCheckerStatus(VAR(Cdd_Adc_CheckerIntEvtType,AUTOMATIC) IntEvt,\
 ...
 *
 * This service clears the safety checker status flags that generated the specified event from the specified interrupt 
 * event instance
 *
 * \param[in] IntEvt    is the numeric ID of the interrupt event instance ID
 * \param[in] CheckerFlag    Pointer to the array of the status flag
 ...
 *
 * This service returns the delay between the the SOC trigger and the actual start of the SOC acquisition of the 
 * specified PPB in SYSCLK cycles.
 *
 * \param[in] PpbId       Numeric ID of the PPB unit
 * \pre None
 * \post None
 ...
 *
 *********************************************************************************************************************/
FUNC(Std_ReturnType,CDD_ADC_CODE) Cdd_Adc_SetResolution(VAR(Cdd_Adc_HwUnitInstanceType,AUTOMATIC)HwUnit,\
