...
#if ((CDD_ECAP_AR_RELEASE_MAJOR_VERSION != (4U)) || \
     (CDD_ECAP_AR_RELEASE_MINOR_VERSION != (3U)) || \
     (CDD_ECAP_AR_RELEASE_REVISION_VERSION != (1U)))
    #error "Cdd_Ecap: AUTOSAR Version Numbers of Cdd Ecap are different!!"
#endif

/** \brief AUTOSAR version information check has to match definition in header file */
#if ((CDD_ECAP_SW_MAJOR_VERSION != (1U)) || \
     (CDD_ECAP_SW_MINOR_VERSION != (0U)) || \
     (CDD_ECAP_SW_PATCH_VERSION != (0U)))
...
static Std_ReturnType Cdd_Ecap_checkStartTimestampErrors(Cdd_Ecap_ChannelType Channel,
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_Init(P2CONST(Cdd_Ecap_ConfigType, AUTOMATIC, CDD_ECAP_CFG) ConfigPtr)
{
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_DeInit(void)
{
...
#endif /* (STD_ON == CDD_ECAP_DEV_ERROR_DETECT) */
    {
        ...
        for (chIdx = 0U; chIdx < CDD_ECAP_HW_CNT; chIdx++)
        {
            ...
            if (FALSE == Cdd_Ecap_ChObj[chIdx].IsRunning)
            {
                /* Disable and Clear Interrupts */
                Cdd_Ecap_intrDisable(Cdd_Ecap_ChObj[chIdx].baseAddr, CDD_ECAP_INT_ALL);
                Cdd_Ecap_intrStatusClear(Cdd_Ecap_ChObj[chIdx].baseAddr, CDD_ECAP_INT_ALL);
                /* Disable CAP1-CAP4 register loads */
                Cdd_Ecap_captureLoadingDisable(Cdd_Ecap_ChObj[chIdx].baseAddr);

                Cdd_Ecap_ResetChObj(&Cdd_Ecap_ChObj[chIdx]);
            }
        }
    }
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_DisableNotification( Cdd_Ecap_ChannelType Channel)
{
...
#endif /* (STD_ON == CDD_ECAP_DEV_ERROR_DETECT) */
    {
        ...
        Cdd_Ecap_ChObj[Channel].NotificationEnabled = FALSE;
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_EnableNotification( Cdd_Ecap_ChannelType Channel)
{
...
#endif /* (STD_ON == CDD_ECAP_DEV_ERROR_DETECT) */
    {
        ...
        SchM_Enter_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
        Cdd_Ecap_ChObj[Channel].NotificationEnabled = TRUE;
        SchM_Exit_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
    }
                                                          ...
                                                          Cdd_Ecap_ActivationType Activation)
{
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        Cdd_Ecap_ChObj[Channel].activation_edge = Activation;
#if ((CDD_ECAP_EDGE_DETECT_API == STD_ON) || (CDD_ECAP_SIGNAL_MEASUREMENT_API == STD_ON))
        Cdd_Ecap_ChObj[Channel].InputState = CDD_ECAP_IDLE;
#endif
...
FUNC(Cdd_Ecap_InputStateType, CDD_ECAP_CODE) Cdd_Ecap_GetInputState(Cdd_Ecap_ChannelType Channel)
{
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        retState = Cdd_Ecap_ChObj[Channel].InputState;
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_EnableEdgeDetection(Cdd_Ecap_ChannelType Channel)
{
...
#endif /* (STD_ON == CDD_ECAP_DEV_ERROR_DETECT) */
    {
        ...
        Cdd_Ecap_ChObj[Channel].IsRunning = TRUE;
        if ((Cdd_Ecap_ChObj[Channel].activation_edge == CDD_ECAP_RISING_EDGE) ||
            (Cdd_Ecap_ChObj[Channel].activation_edge == CDD_ECAP_FALLING_EDGE))
        {
            Cdd_Ecap_ConfigEcap(baseAddr, Cdd_Ecap_ChObj[Channel].activation_edge,
                                CDD_ECAP_DELTA_MODE, TRUE, CDD_ECAP_CEVT1_INT, Channel);
        }
        else
        {
            Cdd_Ecap_ConfigEcap(baseAddr, Cdd_Ecap_ChObj[Channel].activation_edge,
                                CDD_ECAP_DELTA_MODE, TRUE, CDD_ECAP_CEVT1_CEVT2_INT, Channel);
        }
                                                  ...
                                                  uint16 BufferSize, uint16 NotifyInterval)
{
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        Cdd_Ecap_ChObj[Channel].IsRunning = TRUE;
        Cdd_Ecap_ChObj[Channel].IsActive = TRUE;
        if ((Cdd_Ecap_ChObj[Channel].activation_edge == CDD_ECAP_RISING_EDGE) ||
            (Cdd_Ecap_ChObj[Channel].activation_edge == CDD_ECAP_FALLING_EDGE))
        {
            Cdd_Ecap_ConfigEcap(baseAddr, Cdd_Ecap_ChObj[Channel].activation_edge,
                                CDD_ECAP_ABSOLUTE_MODE, TRUE, CDD_ECAP_CEVT1_INT, Channel);
        }
        else
        {
            Cdd_Ecap_ConfigEcap(baseAddr, Cdd_Ecap_ChObj[Channel].activation_edge,
                                CDD_ECAP_ABSOLUTE_MODE, TRUE, CDD_ECAP_CEVT1_CEVT2_INT, Channel);
        }
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_StopTimestamp(Cdd_Ecap_ChannelType Channel)
{
   ...
   retVal = Cdd_Ecap_checkStopTimestampErrors(Channel);
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        if ((FALSE == Cdd_Ecap_ChObj[Channel].IsActive) && (FALSE == Cdd_Ecap_ChObj[Channel].IsRunning))
        {
#if (STD_ON == CDD_ECAP_DEV_ERROR_DETECT)
        ...
        else
        {
            ...
            Cdd_Ecap_ChObj[Channel].IsRunning = FALSE;
            Cdd_Ecap_ChObj[Channel].IsActive = FALSE;
...
FUNC(Cdd_Ecap_IndexType, CDD_ECAP_CODE) Cdd_Ecap_GetTimestampIndex(Cdd_Ecap_ChannelType Channel)
{
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        if ((Cdd_Ecap_ChObj[Channel].IsActive) != (uint32)0U)
        {
            index = Cdd_Ecap_ChObj[Channel].NextTimeStampIndex;
        }
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_ResetEdgeCount(Cdd_Ecap_ChannelType Channel)
{
    ...
    {
        SchM_Enter_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
        Cdd_Ecap_ChObj[Channel].EdgeCounter = 0U;
        SchM_Exit_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
    }
}
...
FUNC(Cdd_Ecap_EdgeNumberType, CDD_ECAP_CODE) Cdd_Ecap_GetEdgeNumbers(Cdd_Ecap_ChannelType Channel)
{
    ...
    {
        SchM_Enter_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
        edgecount= Cdd_Ecap_ChObj[Channel].EdgeCounter;
        SchM_Exit_Cdd_Ecap_CDD_ECAP_EXCLUSIVE_AREA_0();
    }
...
FUNC(void, CDD_ECAP_CODE) Cdd_Ecap_StartSignalMeasurement(Cdd_Ecap_ChannelType Channel)
{
...
#endif /* (STD_ON == CDD_ECAP_DEV_ERROR_DETECT) */
    {
        ...
        Cdd_Ecap_ConfigEcap(baseAddr, CDD_ECAP_BOTH_EDGES,
                            CDD_ECAP_DELTA_MODE, TRUE, CDD_ECAP_CEVT1_CEVT3_INT, Channel);
...
FUNC(Cdd_Ecap_ValueType, CDD_ECAP_CODE) Cdd_Ecap_GetTimeElapsed(Cdd_Ecap_ChannelType Channel)
{
   ...
   retVal = Cdd_Ecap_checkGetTimeElapsedErrors(Channel);
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        if (Cdd_Ecap_ChObj[Channel].PeriodAcquired == TRUE)
        {
            switch (Cdd_Ecap_ChObj[Channel].chCfg.signalMeasurementProperty)
            {
                case CDD_ECAP_DUTY_CYCLE:
                    elapsedTime = ((Cdd_Ecap_ChObj[Channel].HighTime*(100U))/Cdd_Ecap_ChObj[Channel].Period);
                    break;
                case CDD_ECAP_PERIOD_TIME:
                    elapsedTime = Cdd_Ecap_ChObj[Channel].Period;
                    break;
                case CDD_ECAP_HIGH_TIME:
                    elapsedTime = Cdd_Ecap_ChObj[Channel].HighTime;
                    break;
                case CDD_ECAP_LOW_TIME:
                    elapsedTime = Cdd_Ecap_ChObj[Channel].LowTime;
                    break;
                default:
                    elapsedTime = 0U;
                    break;
                                                       ...
                                                       Cdd_Ecap_DutyCycleType* DutyCycleValues)
{
    ...
    if (((Std_ReturnType) E_OK) == retVal)
    {
        ...
        if (Cdd_Ecap_ChObj[Channel].DutyAcquired == TRUE)
        {
            DutyCycleValues->PeriodTime = Cdd_Ecap_ChObj[Channel].DutyCycle.PeriodTime;
            DutyCycleValues->ActiveTime = Cdd_Ecap_ChObj[Channel].DutyCycle.ActiveTime;
...
static void Cdd_Ecap_CheckInit_Internal(uint32 chNum, const Cdd_Ecap_ConfigType *Cdd_Ecap_ConfigPtr)
{
    ...
    Cdd_Ecap_ResetChObj(&Cdd_Ecap_ChObj[chNum]);
    /* Copy the configuration */
    Cdd_Ecap_CopyConfig(&Cdd_Ecap_ChObj[chNum], &Cdd_Ecap_ConfigPtr->chCfg[chNum]);
    /* HW Channel Init */
    Cdd_Ecap_HwUnitInit(&Cdd_Ecap_ChObj[chNum]);

#if (STD_ON == CDD_ECAP_TIMESTAMP_API)
    Cdd_Ecap_ChObj[chNum].IsActive = FALSE;
#endif
                                                                 ...
                                                                 Cdd_Ecap_ActivationType Activation)
{
    ...
    if ((retVal == (Std_ReturnType)E_OK) &&
        (Activation != CDD_ECAP_RISING_EDGE) &&
        (Activation != CDD_ECAP_FALLING_EDGE) &&
        (Activation != CDD_ECAP_BOTH_EDGES))
    {
        (void)Det_ReportError(CDD_ECAP_MODULE_ID, CDD_ECAP_INSTANCE_ID, CDD_ECAP_SETACTIVATIONCONDITION_ID,
                                                         ...
                                                         uint16 NotifyInterval)
{
    ...
    if ((retVal == (Std_ReturnType)E_OK) &&
        (CDD_ECAP_MODE_TIMESTAMP != Cdd_Ecap_ChObj[Channel].chCfg.measurementMode))
    {
        (void)Det_ReportError(CDD_ECAP_MODULE_ID, CDD_ECAP_INSTANCE_ID, CDD_ECAP_STARTTIMESTAMP_ID,
                              CDD_ECAP_E_PARAM_CHANNEL);
        retVal = E_NOT_OK;
