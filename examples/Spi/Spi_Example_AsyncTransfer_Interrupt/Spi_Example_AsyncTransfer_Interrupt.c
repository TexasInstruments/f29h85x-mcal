 ...
 *  FILE DESCRIPTION
 *  ------------------------------------------------------------------------------------------------------------------
 *  File:       Spi_Example_AsyncTransfer_Interrupt.c
 *  Generator:  None
 *
 *  Description:  This file contains SPI example to transmit data asynchronously in Interrupt method in LEVEL 2, and 
 *                  validate the receive data with respect to the transmit data.
 *                
 *        Below Functions are called in this Example:
 *                1. SPIA instance is configured controller mode with loopback enabled, 
 *                2. SPIA Rx interrupt is configured in CAT-1 RT Interrupt
 *                3. Init clock using MCU module
 *                4. Init UART pins using PORT module
 *                5. Call Spi_Init() to init SPI Driver
 *                6. Set AsyncMode to Interrupt using Spi_SetAsyncMode()
 *                7. Initialize Internal and External Buffers using Spi_WriteIB() 
 *                       and Spi_SetupEB() respectively.
 *                8. Call Spi_AsyncTransmit() with sequence ID and wait for transmission to complete.
 *                9. validate Job and sequence results.
 *               10. Validate the receive data with respect to the transmit data.
 * 
...
#include "Spi_Example_AsyncTransfer_Interrupt.h"
...
#define READ_WRITE_WAIT_TIME   1000000000U
/*********************************************************************************************************************
...
int main(void)
{
    Std_ReturnType asyncTransmitStatus;
    ...
    do
    {
        ...
        {
            AppUtils_Printf("Error! Sequence transmission Timeout\n");
            returnValue = E_NOT_OK;
            break;
        }
        else
...
/*********************************************************************************************************************
 *  End of File: Spi_Example_AsyncTransfer_Interrupt.c
 *********************************************************************************************************************/
