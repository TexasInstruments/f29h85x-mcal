...
int main(void)
{
    ...
    AppUtils_Printf("Can_Example_Classic : Sample Application - STARTS !!!\n");
    ...
    AppUtils_Printf("Can_Example_Classic : Can Controller: MCAN A\n\r");
    PduInfo         = &Pdu;
    hth             = 0U;
    ...
    AppUtils_Printf("Can_Example_Classic : Transmitted ID value: 0x%x and length : %d \n\r",PduInfo->id,PduInfo->length);
    AppUtils_Printf("Can_Example_Classic : Transmission data: ");
    for (loopCnt = 0U; loopCnt < Pdu.length; loopCnt++)
    {
    ...
    AppUtils_Printf("Can_Example_Classic : Can Transmission Started \n\r");
    status = Can_Write(hth, PduInfo);        
    if (status != E_OK)
    {
       AppUtils_Printf("Can_Example_Classic : Error In Can_Write.\n\r");
        gTestPassed = E_NOT_OK;
    }
    ...
    if (E_OK == gTestPassed)
    {
        ...
        {
            AppUtils_Printf("Can_Example_Classic : Received Message Id is 0x%X\n\r", CanIf_Idval);
            for (loopCnt = 0U; loopCnt < PduInfo->length; loopCnt++)
            {
                if (CanIf_Rx_Data[loopCnt] != Can_DataRx[loopCnt])
                {
                    gTestPassed = E_NOT_OK;
                    AppUtils_Printf("Can_Example_Classic : Received data mismatched.\n\r");
                }
            }
        }
        else
    ...
    if (gTestPassed == E_OK)
    {
        AppUtils_Printf("Can_Example_Classic : Sample Application success\n\r");
    }
    else
    {
        AppUtils_Printf("Can_Example_Classic : Sample Application Failed\n\r");
    }
    return status;
}
