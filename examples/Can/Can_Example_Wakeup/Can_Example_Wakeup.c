 ...
 *  FILE DESCRIPTION
 *  ------------------------------------------------------------------------------------------------------------------
 *  File:         Can_Example_Wakeup.c
 *  Generator:    None
 *
 *  Description:  This file contains examples for CAN Classic and FD, enable one controller with 2 hardware objects 
 *  for Tx and 2 hardware objects in Rx. validate the receive data with respect to the transmit data, 
 *  below are sets on this example code.
 *                1. Initialize the Can module.
 *                2. Set controller state to STARTED.
 *                3. Set PCAN with the same setting.
 *                      Mode: CAN, Clock Freq: 80 MHz, DataBase Entry: 1 Mbits/s, BitRate[kbit\s]: 1000,
 *                      SamplePoint: 60%, Pre-scaler:8, TSEG1:5, TSEG2:4, tq: 100 ns, Nq: 10
 *                      Sync Jump Width: 4,
 *                      Pre-scaler: 10
 *                4. Start sending predefined data from PCAN
 *                      CAN-ID: 101h, Length: 8, Data(in Hex): D1 1D FF FF F1 1F E1 1E 
 ...
 *                8. Set controller state to SLEEP.
 *                9. Start sending predefined data from PCAN
 *                10. Check for Wakeup event detection.
 *                11. If wakeup event is detected then read rx indication,rx indication should be zero.
 *********************************************************************************************************************/
...
int main(void)
{
    ...
    AppUtils_Printf("Can_Example_Wakeup : Can Controller: MCAN A\n\r");
    PduInfo         = &Pdu;
    hth             = 0U;
    ...
    AppUtils_Printf("Can_Example_Wakeup : Transmitted ID value: 0x%x and length : %d \n\r",PduInfo->id,PduInfo->length);
    AppUtils_Printf("Can_Example_Wakeup : Transmission data: ");
    for (loopCnt = 0U; loopCnt < Pdu.length; loopCnt++)
    {
    ...
    AppUtils_Printf("Can_Example_Wakeup : Can Transmission Started \n\r");
    status = Can_Write(hth, PduInfo);        
    if (status != E_OK)
    {
       AppUtils_Printf("Can_Example_Wakeup : Error In Can_Write.\n\r");
        gTestPassed = E_NOT_OK;
    }
    ...
    if (E_OK == gTestPassed)
    {
        ...
        if ((8U == CanIf_Dlc) && (0x101 == CanIf_Idval))
        {
            AppUtils_Printf("Can_Example_Wakeup : Received Message Id is 0x%X\n\r", CanIf_Idval);
            for (loopCnt = 0U; loopCnt < PduInfo->length; loopCnt++)
            {
                if (CanIf_Rx_Data[loopCnt] != Can_DataRx[loopCnt])
                {
                    gTestPassed = E_NOT_OK;
                    AppUtils_Printf("Can_Example_Wakeup : Received data mismatched.\n\r");
                }
            }
        }
        else
    ...
    Can_SetControllerMode(0U, CAN_CS_STOPPED);
    AppUtils_Printf("Can_Example_Wakeup : Changing MCAN state to SLEEP\n\r");
    /* Changing state to CAN_CS_SLEEP */
    Can_SetControllerMode(0U, CAN_CS_SLEEP);

    AppUtils_Printf("Can_Example_Wakeup : Send the data from PCAN.\n\r");
    AppUtils_Printf("Please press 1 after sending data from PCAN\n\r");
    ...
    /* Check for can wakeup event */
    if((Can_WakeUp_Signal == 1) && (CanIf_RxConfirmationCount == 0))
    {
        gTestPassed = E_OK;
        AppUtils_Printf("Can_Example_Wakeup : Can wakeup event occured\n\r");
    }
    else
    {
        gTestPassed = E_NOT_OK;
        AppUtils_Printf("Can_Example_Wakeup : No Can wakeup event occured\n\r");
    }

    if (gTestPassed == E_OK)
    {
        AppUtils_Printf("Can_Example_Wakeup : Sample Application success\n\r");
    }
    else
    {
        AppUtils_Printf("Can_Example_Wakeup : Sample Application Failed\n\r");
    }

    return status;
}

void EcuM_Can_CheckWakeup(EcuM_WakeupSourceType WakeupSource)
{
    Can_WakeUp_Signal = 1;
}