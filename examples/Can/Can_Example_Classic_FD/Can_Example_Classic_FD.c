 ...
 *                      CAN-ID: 64h,Can Mode: Classic Length: 8, Data(in Hex): D1 1D FF FF F1 1F E1 1E 
 ...
 *                      Mode: CAN, Clock Freq: 80 MHz, DataBase Entry: 5 Mbits/s, BitRate[kbit\s]: 5000,
 *                      SamplePoint: 56.3%, Pre-scaler:1, TSEG1:8, TSEG2:7, tq: 12.5 ns, Nq: 16
 *                      Sync Jump Width: 7,
 *                      Pre-scaler: 1
...
int main(void)
{
    ...
    AppUtils_Printf("Can_Example_Classic_FD : Mode: Can classic, Transmitted ID value: 0x%x, \
    ...
    if(gTestPassed == E_OK)
    {
        ...
        AppUtils_Printf("Can_Example_Classic_FD : Mode: Can Classic, ID value: 0x%x,\
         and length : %d \n\r",PduInfo->id,PduInfo->length);
    ...
    if(gTestPassed == E_OK)
    {
        ...
        PduInfo->length = 64;
        AppUtils_Printf("Can_Example_Classic_FD : Mode: Can FD, Transmitted ID value: 0x%x, \
        and length : %d \n\r",PduInfo->id,PduInfo->length);
        AppUtils_Printf("Can_Example_Classic_FD : Transmission data: ");
        for (loopCnt = 0U; loopCnt < Pdu.length; loopCnt++)
        {
        ...
        {
            AppUtils_Printf("Can_Example_Classic_FD : Send the data from PCAN as below \n\r");
            AppUtils_Printf("Can_Example_Classic_FD : Mode: Can FD, ID value:\
             0x%x, and length : %d \n\r",PduInfo->id,PduInfo->length);
            AppUtils_Printf("Can_Example_Classic_FD : data: ");
            for (loopCnt = 0U; loopCnt < Pdu.length; loopCnt++)
            {
