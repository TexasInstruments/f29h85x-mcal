 ...
 *  ADC instance A. Delay stamp is also printed which the number of system clock cycles delay
 *  between the group trigger and the actual start of the group conversion.
 *
 * Setup required for the example:
 * Connect external voltage to VREFHIAB pin and VREFLOAB to GND.
 * Connect the external voltage to ADCAIN2,ADCAIN3 pins whose difference is considered as the input
 * for the conversion. Similarly, connect the external voltage to ADCAIN4 and ADCAIN5 pins and
 * observe the ADC conversion results from the buffer. Connect the external available voltage to
 * ADCBIN4,ADCBIN5 pins whose difference is considered as the input for the conversion. Similarly,
 * connect the external voltage to ADCBIN6 and ADCBIN7 pins and observe the ADC conversion results
 * from the buffer.
 *
...
void Cdd_Adc_Grp2Notification()
{
    if (Cdd_Adc_GetGroupStatus(CddAdcConf_CddAdcHwUnit_2_CddAdcGroup_0) == CDD_ADC_STREAM_COMPLETED)
    {
        ...
        AppUtils_Printf("Delaystamp for PPB%d is %d\n", CddAdcConf_CddAdcHwUnit_2_CddAdcPpbConfig_0,
                        Cdd_Adc_DelayStamp);
