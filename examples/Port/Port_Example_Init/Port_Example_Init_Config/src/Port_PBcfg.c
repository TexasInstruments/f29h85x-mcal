
/*********************************************************************************************************************
 *  COPYRIGHT
 *  ------------------------------------------------------------------------------------------------------------------
 *  \verbatim
 *
 *                 TEXAS INSTRUMENTS INCORPORATED PROPRIETARY INFORMATION
 *
 *                 Property of Texas Instruments, Unauthorized reproduction and/or distribution
 *                 is strictly prohibited.  This product  is  protected  under  copyright  law
 *                 and  trade  secret law as an  unpublished work.
 *                 (C) Copyright 2024 Texas Instruments Inc.  All rights reserved.
 *
 *  \endverbatim
 *  ------------------------------------------------------------------------------------------------------------------
 *  FILE DESCRIPTION
 *  ------------------------------------------------------------------------------------------------------------------
 *  File:       Port_PBcfg.c
 *  Project:    C29x MCAL
 *  Module:     Port Driver
 *  Generator:  Elektrobit Tresos
 *
 *  Description:  This file contains generated post build configuration data
 *
 *********************************************************************************************************************/
/* ECUCs and Private Configuration JIRA IDs */
/*
 *Design: MCAL-22366,MCAL-22367,MCAL-22368,MCAL-22373,MCAL-22374,MCAL-22375,MCAL-22376,MCAL-22377,
 *Design: MCAL-22378,MCAL-22379,MCAL-22380,MCAL-22381,MCAL-22406,MCAL-22410,MCAL-22407,MCAL-23153,
 *Design: MCAL-23154,MCAL-23155,MCAL-23156,MCAL-23157,MCAL-23158,MCAL-23159,MCAL-23160,MCAL-22312
 */

 /*
 *Design: MCAL-22408
 */
 /* PIN PACKAGE SELECTED = F29H85x_256ZEX */

/*********************************************************************************************************************
 * Header Files
 *********************************************************************************************************************/
#include "Port.h"
 
/*********************************************************************************************************************
 * Version Check (if required)
 *********************************************************************************************************************/
/*****************************************************************************
 *
 * AUTOSAR version information check.
 *
 *****************************************************************************/
#if ((PORT_SW_MAJOR_VERSION != (1U)) || (PORT_SW_MINOR_VERSION != (0U)))
   #error "Version numbers of Port_PBcfg.c and Port.h are inconsistent!"
#endif

#if ((PORT_CFG_MAJOR_VERSION != (1U)) || (PORT_CFG_MINOR_VERSION != (0U)))
   #error "Version numbers of Port_PBcfg.c and Port_Cfg.h are inconsistent!"
#endif

/*********************************************************************************************************************
 * Local Preprocessor #define Constants
 *********************************************************************************************************************/

/*********************************************************************************************************************
 * Local Preprocessor #define Macros
 *********************************************************************************************************************/

/*********************************************************************************************************************
 * Local Type Declarations
 *********************************************************************************************************************/

 /*********************************************************************************************************************
 * Exported Object Definitions
 *********************************************************************************************************************/
#define PORT_START_SEC_CONFIG_DATA
#include "Port_MemMap.h"


static CONST( Port_PinConfigType, PORT_CONFIG_DATA) Port_PortConfigSet_PinConfig[10] =
{
   
   {
    /* .Port_PhysicalPinId = A8U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)0U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_HIGH,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_0_GPIO0,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_STD_PULLUP,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) TRUE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_0_GPIO0
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_0_MCAND_TX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = N14U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)36U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) FALSE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_HIGH,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_36_GPIO36,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_NA,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_36_GPIO36
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_36_MCANC_RX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = H16U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)65U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_NA,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_65_GPIO65,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_STD,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_65_GPIO65
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_65_UARTF_RX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = B4U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)100U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_LOW,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_100_GPIO100,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_OPEN_DRAIN_PULLUP,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_100_GPIO100
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_100_EMIF1_D24
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = M3U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)168U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_IN,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_NA,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_168_AIO168,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_NA,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(1UL<<1U)
                            |(0UL<<2U)
                            |(1UL<<3U)
                            |(0UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_168_AIO168
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_168_SD3_C3
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = M16U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)219U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_HIGH,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_219_GPIO219,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_STD_PULLUP,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_219_GPIO219
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_219_OUTPUTXBAR1
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = M9U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)237U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_OUT,
    .Port_DirectionChangeable = (boolean) TRUE,
    .Port_ModeChangeable = (boolean) TRUE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_LOW,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_237_GPIO237,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_NA,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_SYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(1UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(0UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)2U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_237_GPIO237
        },
        
        [1] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_237_LINA_TX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = B7U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)2U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_NA,
    .Port_DirectionChangeable = (boolean) FALSE,
    .Port_ModeChangeable = (boolean) FALSE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_NA,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_2_UARTA_TX,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_STD,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_ASYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)1U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_2_UARTA_TX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = C7U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)3U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_NA,
    .Port_DirectionChangeable = (boolean) FALSE,
    .Port_ModeChangeable = (boolean) FALSE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_NA,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_DISABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_3_UARTA_RX,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_STD_PULLUP,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_ASYNC,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(0UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(1UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)1U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_3_UARTA_RX
        },
    }

   },
   
   {
    /* .Port_PhysicalPinId = G3U,    Physical Pin Number*/
	.Port_PinId = (Port_PinType)228U,
    .Port_PinDirection = (Port_PinDirectionType)PORT_PIN_NA,
    .Port_DirectionChangeable = (boolean) FALSE,
    .Port_ModeChangeable = (boolean) FALSE,
    .Port_PinLevelValue = (Port_PinLevelValueType)PORT_PIN_LEVEL_NA,
    .Port_ControllerSpecific =
    {
        .Port_AnalogMode = (Port_AnalogModeType)PORT_ANALOG_ENABLED,
        .Port_InitialMuxMode = (Port_PinType)PORT_PIN_MUXMODE_NA,
        .Port_PinPadConfig = (Port_PinPadConfigType)PORT_PIN_TYPE_NA,
        .Port_PinQualification = (Port_PinQualificationMode)PORT_QUAL_NA,
        .Port_PinQualificationPeriod = (uint32)1U,
        .Port_CoreSelect = (Port_PinCoreSelect)PORT_CORE_CPU1,
        .Port_PinConfigFlags = (Port_PinConfigFlagType)((1UL<<0U)
                            |(1UL<<1U)
                            |(0UL<<2U)
                            |(0UL<<3U)
                            |(0UL<<4U)),
        .Port_EnableWakeupPinLPM = (boolean) FALSE
    },
    .Port_NumPortMuxModes = (uint32)1U,
    .Port_PinMode =
    {
        
        
        [0] = 
        {
        .Port_PinMuxMode = (Port_PinModeType)PORT_PIN_MUXMODE_NA
        },
    }

   },

};



const struct Port_ConfigType_s Port_PortConfigSet = 
{
    .Port_PinConfig = (const Port_PinConfigType *)&Port_PortConfigSet_PinConfig[0],
    .Port_NumberOfPortPins = (Port_PinType)10U
};


/*********************************************************************************************************************
 * Local Object Definitions
 *********************************************************************************************************************/

/*********************************************************************************************************************
 *  Local Function Prototypes
 *********************************************************************************************************************/
 
/*********************************************************************************************************************
 *  Local Inline Function Definitions and Function-Like Macros
 *********************************************************************************************************************/

/*********************************************************************************************************************
 *  External Functions Definition
 *********************************************************************************************************************/

/*********************************************************************************************************************
 *  Local Functions Definition
 *********************************************************************************************************************/
#define PORT_STOP_SEC_CONFIG_DATA
#include "Port_MemMap.h"
/*********************************************************************************************************************
 *  End of File: Port_PBcfg.c
 *********************************************************************************************************************/
